...
function parseBody(raw: Record<string, unknown>, line: string): SidecarBody | MalformedMessage {
	switch (raw.type) {
		...
		case 'err': {
			...
			if (typeof code !== 'string' || !ERROR_CODES.has(code)) return malformed(`unknown error code "${String(code)}"`, line);
			if (typeof message !== 'string') return malformed('err has no message', line);
			return { type: 'err', code: code as SidecarErrorCode, message, retryable: raw.retryable === true };
		}
		case 'pointer': {
			...
			return { type: 'pointer', sample: { x: sample.x, y: sample.y, output: optionalString(sample.output) } };
		}
		case 'focus': {
		...
		case 'shortcut': {
			...
			if (typeof shortcutId !== 'string' || shortcutId.length === 0) return malformed('shortcut has no id', line);
			return { type: 'shortcut', shortcutId };
		}
		case 'log': {
			const { level, message } = raw;
			if (typeof level !== 'string' || !LOG_LEVELS.has(level)) return malformed(`unknown log level "${String(level)}"`, line);
...
export function decode(line: string): DecodedMessage {
	...
	if (raw.v !== PROTOCOL_VERSION) return malformed(`unsupported protocol version ${String(raw.v)}`, line);
	const id = raw.id;
	if (typeof id !== 'number' || !Number.isInteger(id) || id < 0) return malformed('id must be a non-negative integer', line);
