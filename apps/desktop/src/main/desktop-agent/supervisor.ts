...
export class SidecarSupervisor {
  ...
  start(): Promise<SidecarState> {
    if (this.disposed)
      return Promise.resolve({ status: "failed", detail: "The desktop agent was disposed." });
  ...
  private async launch(): Promise<SidecarState> {
    ...
    if (!executable)
      return this.setState({ status: "failed", detail: detail ?? "No sidecar binary was found." });
    ...
    if (!this.capabilities) {
      const why =
        reply.type === "err" ? reply.message : "the sidecar answered hello with something else";
      this.killChild();
      return this.setState({
        status: "failed",
        detail: `The sidecar did not report its capabilities: ${why}`,
      });
    }
  ...
  send(body: HostRequest, timeoutMs = this.options.requestTimeoutMs): Promise<SidecarReply> {
    ...
    if (!child)
      return Promise.resolve(fail("unsupported", "The desktop sidecar is not running.", true));
    ...
    return new Promise<SidecarReply>((resolve) => {
      const timer = setTimeout(() => {
        ...
        resolve(
          fail("timeout", `The sidecar did not answer \`${body.type}\` in ${timeoutMs} ms.`, true),
        );
      }, timeoutMs);
      ...
      try {
        ...
        this.pending.get(id)?.(
          fail(
            "internal",
            `Writing to the sidecar failed: ${cause instanceof Error ? cause.message : String(cause)}`,
          ),
        );
      }
    });
  ...
  private receive(messages: DecodedMessage[]): void {
    for (const message of messages) {
      if (message.type === "malformed") {
        this.options.onLog?.(
          "warn",
          `sidecar sent something unreadable (${message.reason}): ${message.sample}`,
        );
        continue;
      }
      ...
      if (!settle) {
        this.options.onLog?.(
          "warn",
          `sidecar answered request ${message.id}, which nothing is waiting for`,
        );
        continue;
      }
      settle(
  ...
  private handleExit(detail: string): void {
    ...
    this.setState({
      status: "starting",
      detail: `The sidecar exited (${detail}); restarting in ${wait} ms.`,
    });
    setTimeout(() => {
