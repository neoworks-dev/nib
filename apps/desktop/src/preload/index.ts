...
const desktopAgent: DesktopAgentBridge = {
	capabilities: () => ipcRenderer.invoke(desktopAgentChannels.capabilities) as Promise<DesktopCapabilities>,
	...
	focusedApplication: () =>
		ipcRenderer.invoke(desktopAgentChannels.focusedApplication) as Promise<ApplicationIdentity | null>,
	accessibleRegions: () => ipcRenderer.invoke(desktopAgentChannels.accessibleRegions) as Promise<DetectedRegion[]>,
	showOverlay: (spec: OverlaySpec) => ipcRenderer.invoke(desktopAgentChannels.showOverlay, spec) as Promise<boolean>,
	...
	onSidecarState: (listener) => subscribe<SidecarState>(desktopAgentChannels.sidecarStateEvent, listener),
};
