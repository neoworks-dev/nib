...
describe('appRequestFor', () => {
	...
	test('refuses rather than dropping a capture when the app is not serving yet', async () => {
		await expect(appRequestFor(undefined, null)(new Request('http://nib/api/assets'))).rejects.toThrow(
			'not being served',
		);
	});
...
describe('uploadCapture', () => {
	...
	test('removes the temp file, so a screenshot is not left in the runtime directory', async () => {
		await uploadCapture(async () => Response.json({ assetId: 'a.png', contentType: 'image/png', byteLength: 4 }), path);
		expect(existsSync(path)).toBe(false);
	});

	test('removes the temp file even when the store refuses it', async () => {
		await expect(uploadCapture(async () => new Response('too large', { status: 413 }), path)).rejects.toThrow('413');
		expect(existsSync(path)).toBe(false);
	});
	...
	test('a file the sidecar did not write is an error, not a silent empty upload', async () => {
		await expect(uploadCapture(async () => new Response('{}'), join(directory, 'missing.png'))).rejects.toThrow();
	});
});
