...
describe('encode', () => {
	test('every host request survives a JSON round trip', () => {
		const requests: HostRequest[] = [
			...
			{ type: 'overlay.show', spec: { regions: [{ rect: { x: 1, y: 2, width: 3, height: 4 }, tone: 'accent' }] } },
			{ type: 'overlay.update', spec: { regions: [], pointer: { radius: 24, tone: 'muted' }, output: 'DP-1' } },
			{ type: 'overlay.hide' },
			...
			{ type: 'capture', request: { mode: 'region', rect: { x: 0, y: 0, width: 100, height: 50 } } },
			{ type: 'focus.query' },
			{ type: 'focus.subscribe' },
			{ type: 'accessibility.tree', maxNodes: 500 },
			{ type: 'shortcuts.bind', shortcuts: [{ id: 'capture', trigger: 'CTRL+SHIFT+s', description: 'Capture' }] },
			{ type: 'shutdown' },
		];
...
describe('decode', () => {
	...
	test('an ok with no payload is not malformed', () => {
		expect(decode(line({ type: 'ok' }))).toEqual({ v: PROTOCOL_VERSION, id: 1, type: 'ok', payload: null });
	});
	...
	test('reads a pointer sample and an event id of zero', () => {
		const raw = JSON.stringify({ v: PROTOCOL_VERSION, id: EVENT_ID, type: 'pointer', sample: { x: 12, y: 34 } });
	...
	test('a focus event fills every unanswered identity field with null', () => {
		const message = decode(line({ type: 'focus', application: { appId: 'blender' } })) as SidecarMessage;
		expect(message).toEqual({
			v: PROTOCOL_VERSION,
			id: 1,
			type: 'focus',
			application: { appId: 'blender', atspiName: null, desktopEntry: null, title: null, pid: null },
		});
	});

	test.each([
		['unknown type', line({ type: 'nope' }), 'unknown message type "nope"'],
		['wrong version', JSON.stringify({ v: 2, id: 1, type: 'ok' }), 'unsupported protocol version 2'],
		['missing id', JSON.stringify({ v: PROTOCOL_VERSION, type: 'ok' }), 'id must be a non-negative integer'],
		['fractional id', JSON.stringify({ v: PROTOCOL_VERSION, id: 1.5, type: 'ok' }), 'id must be a non-negative integer'],
		['negative id', JSON.stringify({ v: PROTOCOL_VERSION, id: -1, type: 'ok' }), 'id must be a non-negative integer'],
	...
	test('capabilities missing a strategy are malformed, not silently defaulted', () => {
		...
		expect(decode(line({ type: 'capabilities', capabilities: partial }))).toMatchObject({ type: 'malformed' });
	});
});
...
describe('FrameReader', () => {
	...
	test('an event between a request and its reply comes through in order', () => {
		const reader = new FrameReader();
		const chunk = [
			JSON.stringify({ v: PROTOCOL_VERSION, id: EVENT_ID, type: 'log', level: 'info', message: 'up' }),
			JSON.stringify({ v: PROTOCOL_VERSION, id: EVENT_ID, type: 'pointer', sample: { x: 1, y: 2 } }),
			line({ type: 'ok', payload: null }),
		].join('\n');

		const messages = reader.push(`${chunk}\n`);
		expect(messages.map((message) => message.type)).toEqual(['log', 'pointer', 'ok']);
		expect(messages.map((message) => (message.type === 'malformed' ? -1 : message.id))).toEqual([0, 0, 1]);
	});

	test('an escaped newline inside a message does not split the frame', () => {
		const reader = new FrameReader();
		const raw = JSON.stringify({ v: PROTOCOL_VERSION, id: 0, type: 'log', level: 'warn', message: 'one\ntwo' });
		const messages = reader.push(`${raw}\n`);
		expect(messages).toEqual([{ v: PROTOCOL_VERSION, id: 0, type: 'log', level: 'warn', message: 'one\ntwo' }]);
	});
...
describe('payload parsers', () => {
	...
	test('drops a malformed region rather than the whole tree', () => {
		const regions = parseDetectedRegions([
			{ id: 'a', rect: { x: 0, y: 0, width: 10, height: 10 }, source: 'atspi', confidence: 1, label: 'OK' },
			...
			{ id: 'd', rect: { x: 5, y: 5, width: 5, height: 5 }, source: 'pixel', confidence: 0.4, sensitive: true },
		]);
	...
	test('a region with no confidence reads as zero rather than NaN', () => {
		const [region] = parseDetectedRegions([{ id: 'a', rect: { x: 0, y: 0, width: 1, height: 1 }, source: 'pixel' }]);
