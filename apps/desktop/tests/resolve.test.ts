...
import {
  resolveSidecar,
  SIDECAR_NAME,
  SIDECAR_OVERRIDE_VARIABLE,
} from "../src/main/desktop-agent/resolve";
...
describe("the override", () => {
  test("is used when it points at an executable", () => {
    const resolution = resolveSidecar(
      { [SIDECAR_OVERRIDE_VARIABLE]: "/opt/nib/overlay" },
      present("/opt/nib/overlay"),
    );
...
describe("the PATH scan", () => {
  ...
  test("steps over a directory that does not have it", () => {
    const resolution = resolveSidecar(
      { PATH: "/usr/bin:/home/me/.local/bin" },
      present(`/home/me/.local/bin/${SIDECAR_NAME}`),
    );
