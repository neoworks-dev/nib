...
import { type SidecarProcess, SidecarSupervisor } from "../src/main/desktop-agent/supervisor";
...
class FakeSidecar implements SidecarProcess {
  ...
  constructor(
    private readonly respond: (message: HostMessage, sidecar: FakeSidecar) => void = () => {},
  ) {}
...
function helloOnly(message: HostMessage, sidecar: FakeSidecar): void {
  if (message.type === "hello")
    sidecar.emit(line({ id: message.id, type: "capabilities", capabilities: CAPABILITIES }));
}
...
describe("requests", () => {
  test("a reply is matched to its request, not to the newest one", async () => {
    const { supervisor } = harness((message, sidecar) => {
      ...
      if (message.type === "overlay.hide")
        sidecar.emit(line({ id: message.id, type: "ok", payload: "overlay" }));
    });
    await supervisor.start();
  ...
  test("sending with no sidecar running answers rather than throwing", async () => {
    ...
    expect(await supervisor.send({ type: "focus.query" })).toMatchObject({
      type: "err",
      code: "unsupported",
    });
  });
