/**
 * Headless adapter smoke test: starts a session, prints every normalized event
 * as JSONL, auto-allows permission prompts and exits when the turn goes idle.
 *
 *   bun apps/web/scripts/smoke.ts "list the files here"
 *   SMOKE_HARNESS=pi bun apps/web/scripts/smoke.ts "say hi" /tmp/scratch
 */
...
const sessionId = await host.create({
  harnessId: process.env.SMOKE_HARNESS ?? "claude-code",
  cwd,
});
...
const unsubscribe = host.subscribe(sessionId, (event: AnyAgentEvent) => {
  ...
  if (event.type === "session.status" && event.data.status === "error")
    settle(`error: ${event.data.detail}`);
});
