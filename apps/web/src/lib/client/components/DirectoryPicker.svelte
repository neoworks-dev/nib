<script lang="ts">
	...
	const {
		...
		recent,
		onChange,
	}: { value: string; recent: string[]; onChange: (value: string) => void } = $props();
	...
	$effect(() => {
		...
		let current = true;
		void sessions.listDirectories(query).then((listing) => {
	...
	function choose(suggestion: Suggestion) {
		// Keep the picker open on a directory: the next probe lists its children.
		onChange(suggestion.fromHistory ? suggestion.path : `${suggestion.path}/`);
		open = !suggestion.fromHistory;
	}
...
<div class="relative">
	<div
		class="flex items-center gap-2 rounded-lg border border-line bg-input px-3 py-2 focus-within:border-line-strong"
	>
		<span class="text-muted"><FolderOpenIcon size={16} /></span>
		<input
			{value}
			oninput={(event) => {
				open = true;
			...
			spellcheck="false"
			class="min-w-0 flex-1 bg-transparent font-mono text-sm text-default placeholder:text-faint focus:outline-none"
		/>
	</div>
