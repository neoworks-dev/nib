<script lang="ts">
	...
	let drag = $state<{ startX: number; startY: number; pointerX: number; pointerY: number; moved: boolean } | null>(null);
	...
	function hintAt(event: PointerEvent): { frameId: string; edge: 'left' | 'right' | 'top' | 'bottom' } | null {
	...
	function start(event: PointerEvent) {
		...
		drag = { startX: event.clientX, startY: event.clientY, pointerX: event.clientX, pointerY: event.clientY, moved: false };
...
<div class="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
	...
	>
		...
		{:else}
			<p class="px-3 py-2 text-xs text-dim">No plugin provides the pane “{instance?.paneId ?? instanceId}”.</p>
		{/if}
	</div>
</div>
