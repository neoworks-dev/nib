<script lang="ts">
  ...
  import { moveRect, type ResizeHandle, resizeRect, snapAt, snapRect } from "../layout/windows";
  ...
  const hint = $derived<PaneEdge | null>(
    paneDrag.hint?.frameId === frame.frameId ? paneDrag.hint.edge : null,
  );
  ...
  let drag = $state<{ handle: ResizeHandle | "move"; pointerX: number; pointerY: number } | null>(
    null,
  );
...
<section
  ...
  class="absolute z-overlay flex flex-col overflow-hidden rounded-xl border bg-elevated shadow-2xl {focused
    ? 'border-line-strong'
    : 'border-line'}"
  style="left:{frame.rect.x}px; top:{frame.rect.y}px; width:{frame.rect.width}px; height:{frame.rect
    .height}px;"
...
>
  ...
  {#if hint}
    <div
      class="pointer-events-none absolute border-2 border-action bg-action/20 {hintClass[hint]}"
    ></div>
  {/if}
