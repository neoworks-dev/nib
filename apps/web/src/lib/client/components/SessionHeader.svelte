<script lang="ts">
  ...
  const workspace = $derived(
    session.cwd ? workspaceName(session.cwd) : (session.harnessId ?? "workspace"),
  );
...
<header class="flex flex-wrap items-center gap-2 px-2 py-2">
  <nav class="flex min-w-0 items-center gap-2 text-sm" aria-label="Breadcrumb">
    ...
    {:else}
      <button
        type="button"
        class="group flex min-w-0 items-center gap-1.5 text-left"
        onclick={startRename}
      >
        <span class="truncate font-medium text-default">{task}</span>
        <span
          class="shrink-0 text-faint opacity-0 transition-opacity duration-fast group-hover:opacity-100"
        >
