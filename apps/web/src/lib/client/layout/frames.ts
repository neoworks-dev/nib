import type { PaneAxis, PaneEdge, PaneNode, PaneRect } from '@nib-ui/ui-contracts';
...
export function insertAtEdge(root: PaneNode, instanceId: string, edge: PaneEdge): PaneNode {
	...
	if (root.kind === 'split' && root.axis === axis) {
		...
		const children = leading(edge) ? [leafNode(instanceId), ...root.children] : [...root.children, leafNode(instanceId)];
 ...
 */
export function insertBeside(root: PaneNode, instanceId: string, targetInstanceId: string, edge: PaneEdge): PaneNode {
	return insertInto(root, instanceId, targetInstanceId, edge) ?? root;
}

function insertInto(node: PaneNode, instanceId: string, targetInstanceId: string, edge: PaneEdge): PaneNode | null {
	if (node.kind === 'leaf') {
		return node.instanceId === targetInstanceId ? insertAtEdge(node, instanceId, edge) : null;
	}

	const index = node.children.findIndex((child) => child.kind === 'leaf' && child.instanceId === targetInstanceId);
...
export function retainLeaves(node: PaneNode, keep: (instanceId: string) => boolean): PaneNode | null {
 ...
 */
export function resizeSiblings(sizes: readonly number[], index: number, delta: number, minimum = 0): number[] {
...
export function minimumFraction(axis: PaneAxis, extent: number, count: number): number {
	...
	const minimum = axis === 'row' ? MIN_WINDOW_WIDTH : MIN_WINDOW_HEIGHT;
	return Math.min(minimum / extent, 1 / count);
}

/**
 * What a title bar drag does. A leaf that is the whole frame carries the frame
 * with it, as a plain window does; once a frame holds more than one leaf a drag
 * on a leaf gives that leaf up, and the frame gets a strip of its own to be
 * dragged by instead.
 */
export interface FrameChrome {
	frameBar: boolean;
	leafDrag: 'move' | 'detach';
}

export function frameChrome(root: PaneNode): FrameChrome {
	const shared = listLeaves(root).length > 1;
	return { frameBar: shared, leafDrag: shared ? 'detach' : 'move' };
}

export function pointInRect(rect: PaneRect, x: number, y: number): boolean {
	return x >= rect.x && x <= rect.x + rect.width && y >= rect.y && y <= rect.y + rect.height;
}

/**
 * Which edge a pointer over a frame would attach to. The frame is cut into four
 * triangles about its centre, so exactly one edge is ever the answer.
 */
export function dropEdge(rect: PaneRect, x: number, y: number): PaneEdge {
	const horizontal = rect.width > 0 ? (x - (rect.x + rect.width / 2)) / rect.width : 0;
	const vertical = rect.height > 0 ? (y - (rect.y + rect.height / 2)) / rect.height : 0;
