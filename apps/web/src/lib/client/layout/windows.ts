...
export function defaultRect(bounds: Bounds): WindowRect {
	return clampRect(
		{ x: 24, y: 24, width: Math.round(bounds.width * 0.42), height: Math.round(bounds.height * 0.6) },
		bounds,
	);
}
...
export function resizeRect(rect: WindowRect, handle: ResizeHandle, dx: number, dy: number, bounds: Bounds): WindowRect {
 ...
 */
export function snapAt(x: number, y: number, bounds: Bounds, margin = SNAP_MARGIN): SnapZone | null {
	const left = x <= margin;
	...
	if (x < -margin || y < -margin || x > bounds.width + margin || y > bounds.height + margin) return null;
