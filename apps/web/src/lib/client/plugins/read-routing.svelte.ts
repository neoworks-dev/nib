...
import { type ChatPaneParams, canvasState, chatPaneId } from "@nib-ui/plugin-canvas";
import {
  MODEL_DEFAULT_ORBIT,
  MODEL_DEFAULT_SIZE,
  type ModelObject,
    ...
    const stop = $effect.root(() => {
      $effect(() => {
        for (const instance of panes.instances(chatPaneId)) {
          ...
          const anchored = target.workstream
            ? canvasState.anchorFor(target.workstream.id) !== null
            : false;

          const route = routeRead({
            path: session ? latestReadPath(session) : null,
            attached: attachments
              .siblings(instance.instanceId)
          ...
          untrack(() => {
            if (route.kind === "editor")
              void fileViewer.open(session.sessionId, route.path, { activate: false });
...
async function showModel(sessionId: string, path: string): Promise<void> {
  ...
  const format =
    asset && (modelFormatForContentType(asset.contentType) ?? modelFormatForName(asset.path));
