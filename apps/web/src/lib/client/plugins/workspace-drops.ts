  ...
  apply(ctx) {
      ...
      switch (canvasObjectKindForPath(file.path)) {
        ...
        default:
          canvasState.reportError(
            `nothing on the board draws ${file.path} — opened in the editor instead`,
          );
    ...
    const drop = async (files: WorkspaceFileRef[], at: Point): Promise<boolean> => {
      ...
      if (card) {
        ...
        if (!sessionId) {
          canvasState.reportError(
            "this workstream has no task yet — launch it before attaching files",
          );
          return true;
        }
      ...
      for (const file of files) {
        await place(file, { x, y: at.y }).catch((cause: unknown) => {
          canvasState.reportError(
            `could not read ${file.path}: ${cause instanceof Error ? cause.message : String(cause)}`,
          );
        });
