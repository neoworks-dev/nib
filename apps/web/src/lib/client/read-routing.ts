...
import { type PaneKind, readFilePath } from "@nib-ui/ui-contracts";
...
export type ReadRoute =
  { kind: "editor"; path: string } | { kind: "model"; path: string } | { kind: "none" };
...
export function routeRead({ path, attached, lastRouted, following }: ReadRoutingInput): ReadRoute {
  ...
  if (canvasObjectKindForPath(path) === "model" && attached.includes("model"))
