...
export function parseBoard(raw: unknown, cwd: string): BoardDoc {
	...
	const rev = typeof candidate.rev === 'number' && Number.isFinite(candidate.rev) ? Math.max(0, candidate.rev) : 0;
...
export function parseLayout(value: unknown): PaneLayout | undefined {
	...
	for (const entry of Array.isArray(candidate.instances) ? candidate.instances : []) {
		...
		instances.set(instanceId, keeps ? { instanceId, paneId, params: params as Record<string, unknown> } : { instanceId, paneId });
	}
...
function parseRect(value: unknown): PaneRect | undefined {
	...
	const finite = [x, y, width, height].every((entry) => typeof entry === 'number' && Number.isFinite(entry));
	return finite ? { x: x!, y: y!, width: width!, height: height! } : undefined;
}

function parseNode(value: unknown, instances: Map<string, PaneInstance>, placed: Set<string>): PaneNode | undefined {
	if (!value || typeof value !== 'object') return undefined;
	const node = value as { kind?: unknown; instanceId?: unknown; axis?: unknown; children?: unknown; sizes?: unknown };

	if (node.kind === 'leaf') {
		const instanceId = node.instanceId;
		if (typeof instanceId !== 'string' || !instances.has(instanceId) || placed.has(instanceId)) return undefined;
