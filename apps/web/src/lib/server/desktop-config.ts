...
export function desktopConfigPath(): string {
	...
	return join(base && base.length > 0 ? base : join(homedir(), '.config'), 'nib', 'desktop-agent.json');
}
...
export async function writeDesktopConfig(patch: Record<string, unknown>): Promise<Record<string, unknown>> {
