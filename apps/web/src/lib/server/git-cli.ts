...
export async function status(cwd: string): Promise<GitStatus> {
	const porcelain = await git(cwd, ['status', '--porcelain=v1', '--branch', '--untracked-files=all']);
...
export async function commit(cwd: string, message: string, paths: string[] = []): Promise<GitCommitResult> {
	if (paths.length > 0) {
...
async function numstat(cwd: string, extra: string[]): Promise<Map<string, { added: number; removed: number }>> {
