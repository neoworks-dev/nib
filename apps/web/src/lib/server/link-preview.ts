...
export function parseLinkMetadata(html: string): LinkMetadata {
  ...
  const title =
    open.title ?? twitter.title ?? (titleTag ? decodeEntities(titleTag[1]!).trim() : "");
...
export async function fetchLinkPreview(
  ...
  const copy = async (href: string | undefined): Promise<string | undefined> => {
    ...
    try {
      const image = await guardedFetch(new URL(resolved), MAX_IMAGE_BYTES, "image/*", {
        ...options,
        signal,
      });
  ...
  const [imageAssetId, faviconAssetId] = await Promise.all([
    copy(metadata.image),
    copy(metadata.favicon),
  ]);
  return {
