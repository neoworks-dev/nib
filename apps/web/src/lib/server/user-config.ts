...
export function mergeUserConfig(raw: unknown, patch: Partial<UserConfig>): UserConfig & Record<string, unknown> {
	...
	const lastProject =
		patch.lastProject ?? (typeof storedProject === 'string' && storedProject.length > 0 ? storedProject : null);
