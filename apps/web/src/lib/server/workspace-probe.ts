...
export async function searchFiles(cwd: string, query: string, limit = 20): Promise<FileMatch[]> {
	...
	return fuzzyRank(files, query, (path) => path, limit).map(({ item, score }) => ({ path: item, score }));
}

async function readDirectories(base: string, prefix: string, limit: number): Promise<DirectoryListing> {
