...
describe('sniffAsset', () => {
	...
	it('reads an obj a windows exporter opened with a byte order mark', () => {
		expect(sniffAsset(bytes(0xef, 0xbb, 0xbf, '# blender\r\nv 1 1 -1\r\nf 1 1 1\r\n'))?.contentType).toBe('model/obj');
	});
...
describe('storeAsset', () => {
	...
	describe('with the text fallback', () => {
		...
		it('keeps a payload no signature claims as plain text', async () => {
			...
			expect(await readAsset(directory, stored.assetId)).toMatchObject({ contentType: 'text/plain' });
		});
		...
		it('refuses binary junk, an empty payload, and broken utf-8', async () => {
			expect(storeAsset(directory, bytes('text', 0x00, 'and a nul'), options)).rejects.toBeInstanceOf(
				UnsupportedAssetError,
			);
			expect(storeAsset(directory, new Uint8Array([0xc3, 0x28, 0xff, 0xfe]), options)).rejects.toBeInstanceOf(
				UnsupportedAssetError,
			);
			expect(storeAsset(directory, new Uint8Array(0), options)).rejects.toBeInstanceOf(UnsupportedAssetError);
		});
