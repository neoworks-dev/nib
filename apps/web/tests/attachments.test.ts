...
import { attachmentMetadata, composeAttachmentPrompt } from '../src/lib/server/attachments';
...
import type { AssetService, HarnessAdapter, SessionAttachment, SessionHost } from '../src/lib/server/services';
...
describe('attachmentMetadata', () => {
	test('keeps the store id out of the path and stays absent when there is nothing to record', () => {
		expect(attachmentMetadata([shot])).toEqual([{ assetId: 'aaa.png', mime: 'image/png', name: 'shot.png' }]);
...
describe('session.send dispatch', () => {
	...
	test('an attachment that no longer resolves is dropped and logged, not fatal', async () => {
		...
		expect(host.view(sessionId)!.logs).toEqual([
			{ level: 'warn', message: 'attachment "gone.png" is no longer stored and was not sent', ts: expect.any(Number) },
		]);
	});
});
