...
describe('parseBoard', () => {
	...
	it('drops entries without a kind and an id, which nothing could ever address', () => {
		const parsed = parseBoard({ objects: [{ kind: 'note' }, { id: 'n' }, 7, null, { kind: 'note', id: 'n' }] }, cwd);
	...
	it('keeps the first of two objects sharing an id', () => {
		const parsed = parseBoard(
			{ objects: [{ kind: 'note', id: 'n', body: 'first' }, { kind: 'note', id: 'n', body: 'second' }] },
			cwd,
		);
...
describe('parseBoard layout', () => {
					...
					frames: [
						{
							...
							root: {
								...
								axis: 'row',
								sizes: [0.5, 0.5],
								children: [{ kind: 'leaf', instanceId: 'i1' }, { kind: 'leaf', instanceId: 'ghost' }],
							},
						},
					],
				},
	...
	it('a frame with a broken rect or an unreadable tree is dropped, not the whole layout', () => {
		const parsed = parseBoard(
			{
				layout: {
					...
					frames: [
						{ frameId: 'bad', rect: { x: 0, y: 0, width: 'wide' }, root: { kind: 'leaf', instanceId: 'i1' } },
						{ frameId: 'alsoBad', rect: { x: 0, y: 0, width: 10, height: 10 }, root: { kind: 'tabs' } },
						{ frameId: 'f2', rect: { x: 1, y: 2, width: 300, height: 200 }, root: { kind: 'leaf', instanceId: 'i2' } },
					],
				},
			},
			cwd,
		...
		expect(parsed.layout?.instances).toEqual([{ instanceId: 'i2', paneId: 'chat', params: { sessionId: 's1' } }]);
	});

	it('an instance claimed by two frames is only rendered by the first', () => {
		const frame = { frameId: 'f1', rect: { x: 0, y: 0, width: 300, height: 200 }, root: { kind: 'leaf', instanceId: 'i1' } };
		const parsed = parseBoard(
	...
	it('sizes are repaired to fractions of the split, whatever was stored', () => {
		const parsed = parseBoard(
			{
				layout: {
					instances: layout.instances,
					frames: [{ ...layout.frames[0], root: { ...layout.frames[0]!.root, sizes: [3, 'wide'] } }],
				},
			},
			cwd,
		);
		...
		expect(root?.kind === 'split' && root.sizes.reduce((sum, size) => sum + size, 0)).toBeCloseTo(1);
		expect(root?.kind === 'split' && root.sizes[0]).toBeGreaterThan(root?.kind === 'split' ? root.sizes[1]! : 1);
	});
		...
		const parsed = parseBoard(
			{
				layout: {
					...
					frames: [{ ...layout.frames[0], root: { kind: 'split', axis: 'column', sizes: [1], children: [{ kind: 'leaf', instanceId: 'i1' }] } }],
				},
			},
			cwd,
		);
...
describe('board files', () => {
	...
	it('accepts exactly the next revision and reads it back', async () => {
		await writeBoardFile(directory, board(1, [{ kind: 'workstream', id: 'w', goal: '', x: 0, y: 0 }]));
	...
	it('the pane layout survives the write and reads back with the board', async () => {
		const layout: BoardDoc['layout'] = {
			frames: [{ frameId: 'f1', rect: { x: 4, y: 8, width: 500, height: 300 }, root: { kind: 'leaf', instanceId: 'i1' } }],
			instances: [{ instanceId: 'i1', paneId: 'git' }],
		};
	...
	it('a board saved with no layout keeps the key out of the file', async () => {
		await writeBoardFile(directory, board(1));
		const raw = JSON.parse(readFileSync(join(directory, boardFileName(cwd)), 'utf8')) as Record<string, unknown>;
	...
	it('lists every board, reading each directory out of its document', async () => {
		...
		expect((await listBoardFiles(directory)).map((entry) => entry.cwd).sort()).toEqual(['/home/dev/repo', '/other']);
	});
...
describe('boardSummary', () => {
	...
	it('reads a board written before the review mark existed as unreviewed', () => {
		const summary = boardSummary(board(1, [{ kind: 'workstream', id: 'w1', x: 0, y: 0, goal: 'a', reviewedAt: 'yes' }]));
