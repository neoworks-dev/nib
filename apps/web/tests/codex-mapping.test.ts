...
} from '../src/lib/server/plugins/codex/mapping';

/** Drives the reducer over a stream the way the adapter's read loop does. */
function drain(events: unknown[], state = createCodexStreamState('/repo')): { state: CodexStreamState; events: EmittedEvent[] } {
...
describe('mapCodexEvent', () => {
	...
	test('opens an assistant message and reports working on turn.started', () => {
		...
		expect(ofType(events, 'message.started')[0]?.data).toEqual({ messageId: 'turn-1', role: 'assistant' });
	...
	test('maps a completed agent_message to a started and completed text block', () => {
		const { events } = drain([
			{ type: 'turn.started' },
			{ type: 'item.completed', item: { id: 'item_0', type: 'agent_message', text: 'It printed hello.' } },
		]);
	...
	test('streams only the unseen tail of a text item across item.updated', () => {
		...
		expect(ofType(events, 'block.delta').map((event) => event.data.textDelta)).toEqual(['Hel', 'lo wo', 'rld']);
	});

	test('maps reasoning items to thinking blocks', () => {
		const { events } = drain([
			{ type: 'turn.started' },
			{ type: 'item.completed', item: { id: 'item_0', type: 'reasoning', text: 'Checking the file.' } },
		]);
		expect(ofType(events, 'block.completed')[0]?.data.content).toEqual({ kind: 'thinking', text: 'Checking the file.' });
	});
	...
	test('pairs a command_execution with a tool_result block that shares its tool use id', () => {
		const { events } = drain([
			...
			{
				type: 'item.started',
				item: { id: 'item_1', type: 'command_execution', command: 'cat a.txt', aggregated_output: '', exit_code: null, status: 'in_progress' },
			},
			{
				type: 'item.completed',
				item: { id: 'item_1', type: 'command_execution', command: 'cat a.txt', aggregated_output: 'hello\n', exit_code: 0, status: 'completed' },
			},
		]);
		...
		expect(ofType(events, 'block.completed').at(-1)?.data).toEqual({
			...
			content: { kind: 'tool_result', toolUseId: 'turn-1:item_1', output: 'hello\n', isError: false },
		});
	});
	...
	test('flags a non-zero exit code as a failed tool result', () => {
		const { events } = drain([
			...
			{
				type: 'item.completed',
				item: { id: 'item_0', type: 'command_execution', command: 'false', aggregated_output: '', exit_code: 1, status: 'completed' },
			},
		]);
	...
	test('accumulates usage across turns and closes the message on turn.completed', () => {
		const { state, events } = drain([
			...
			{ type: 'turn.started' },
			{ type: 'turn.completed', usage: { input_tokens: 200, cached_input_tokens: 60, output_tokens: 5 } },
		]);
		...
		expect(ofType(events, 'message.completed').map((event) => event.data.messageId)).toEqual(['turn-1', 'turn-2']);
	...
	test('synthesizes a message when an item arrives without a turn.started', () => {
		const { events } = drain([{ type: 'item.completed', item: { id: 'item_0', type: 'agent_message', text: 'hi' } }]);
		expect(ofType(events, 'message.started')[0]?.data).toEqual({ messageId: 'turn-1', role: 'assistant' });
	});

	test('reports turn.failed and the fatal error event as session errors', () => {
		const failed = drain([{ type: 'turn.started' }, { type: 'turn.failed', error: { message: 'usage limit reached' } }]);
		...
		expect(ofType(fatal.events, 'session.status')[0]?.data).toEqual({ status: 'error', detail: 'stream closed' });
	});
	...
	test('degrades malformed lines to a warning instead of throwing', () => {
		const malformed = [null, undefined, 7, 'not json', [], {}, { type: 9 }, { type: 'item.completed' }, { type: 'item.started', item: { type: 'agent_message' } }];
...
describe('parseModelCache', () => {
	test('keeps listed models and drops internal ones', () => {
		const raw = JSON.stringify({
			models: [
				{ slug: 'gpt-5.6-sol', display_name: 'GPT-5.6-Sol', description: 'Latest', visibility: 'list' },
