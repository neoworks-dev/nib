...
import {
  ...
  readDesktopConfig,
  writeDesktopConfig,
} from "../src/lib/server/desktop-config";
...
describe("the desktop-agent config file", () => {
  ...
  it.each([
    ...
    "answers an empty document for %s, which no rule can be read out of",
  ...
  it("keeps a key a newer build wrote", async () => {
    ...
    const merged = await writeDesktopConfig({
  ...
  it("replaces the rule list wholesale rather than concatenating it", async () => {
    ...
    const merged = await writeDesktopConfig({
      applications: [{ match: { appId: "b" }, policy: "allow" }],
    });
