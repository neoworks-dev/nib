...
describe('insertAtEdge', () => {
	test('a horizontal edge makes a row and a vertical one a column', () => {
		...
		expect(insertAtEdge(leafNode('a'), 'b', 'top')).toMatchObject({ kind: 'split', axis: 'column' });
		expect(insertAtEdge(leafNode('a'), 'b', 'bottom')).toMatchObject({ kind: 'split', axis: 'column' });
	});
...
describe('insertBeside', () => {
	...
	test('a perpendicular edge nests a split inside the target place', () => {
		...
		expect(listLeaves(node)).toEqual(['a', 'b', 'c']);
		expect(node.kind === 'split' && node.children[1]).toMatchObject({ kind: 'split', axis: 'column' });
		expect(node.kind === 'split' && node.sizes).toEqual([0.5, 0.5]);
	});

	test('a target deep in the tree is found', () => {
		const node = insertBeside(row([leafNode('a'), column([leafNode('b'), leafNode('c')], [0.5, 0.5])], [0.5, 0.5]), 'd', 'c', 'bottom');
...
describe('removeLeaf', () => {
	...
	test('what is left shares the space in the proportions it had', () => {
		const node = removeLeaf(row([leafNode('a'), leafNode('b'), leafNode('c')], [0.5, 0.25, 0.25]), 'a');
...
describe('normalizeSizes', () => {
	...
	test('a tree from storage is normalized level by level', () => {
		const node = normalizeNode(row([leafNode('a'), column([leafNode('b'), leafNode('c')], [4, 4])], [1, 3]));
...
describe('frameChrome', () => {
	...
	test('a shared frame gets a strip, and its leaves give themselves up instead', () => {
		...
		expect(frameChrome(row([leafNode('a'), column([leafNode('b'), leafNode('c')], [0.5, 0.5])], [0.5, 0.5]))).toEqual({
			frameBar: true,
			leafDrag: 'detach',
		});
	});
...
describe('addressing', () => {
	...
	test('retaining leaves drops the rest and collapses what is left', () => {
		expect(retainLeaves(tree, (leaf) => leaf !== 'b')).toEqual(row([leafNode('a'), leafNode('c')], [0.5, 0.5]));
