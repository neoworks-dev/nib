...
describe('isBlockedAddress', () => {
	...
	const allowed = ['1.1.1.1', '8.8.8.8', '172.32.0.1', '192.167.1.1', '99.1.1.1', '2606:4700::1111'];
...
describe('assertPublicUrl', () => {
	...
	it('rejects a scheme that is not http', async () => {
		...
		expect(assertPublicUrl(new URL('data:text/html,<b>hi</b>'))).rejects.toBeInstanceOf(BlockedUrlError);
	});

	it('rejects a host that resolves to a private address', async () => {
		expect(assertPublicUrl(new URL('http://intranet.test/'), resolvesTo('10.0.0.5'))).rejects.toBeInstanceOf(
			BlockedUrlError,
		);
	});
	...
	it('rejects a literal private address without consulting dns', async () => {
		expect(assertPublicUrl(new URL('http://127.0.0.1:8080/'), resolvesTo('1.1.1.1'))).rejects.toBeInstanceOf(
			BlockedUrlError,
		);
		expect(assertPublicUrl(new URL('http://[::1]:8080/'), resolvesTo('1.1.1.1'))).rejects.toBeInstanceOf(
			BlockedUrlError,
		);
	});

	it('rejects a host that resolves to nothing', async () => {
		expect(assertPublicUrl(new URL('http://nowhere.test/'), resolvesTo())).rejects.toBeInstanceOf(BlockedUrlError);
	});
});
...
describe('parseLinkMetadata', () => {
	...
	it('falls back to the title tag and the plain description', () => {
		const metadata = parseLinkMetadata('<head><title>Only a title</title><meta name="description" content="d"></head>');

		expect(metadata).toEqual({ title: 'Only a title', description: 'd' });
	});

	it('reports nothing found rather than guessing', () => {
		expect(parseLinkMetadata('<html><body><h1>No head here</h1></body></html>')).toEqual({ title: '' });
	});
	...
	it('accepts shortcut icon and apple touch icon', () => {
		expect(parseLinkMetadata('<head><link rel="shortcut icon" href="/a.ico"></head>').favicon).toBe('/a.ico');
		expect(parseLinkMetadata('<head><link rel="apple-touch-icon" href="/b.png"></head>').favicon).toBe('/b.png');
	});
...
describe('fetchLinkPreview redirects', () => {
		...
		const preview = await fetchLinkPreview('https://start.test/a', directory, {
			...
			fetchImpl: ((input: URL) => {
				...
				return Promise.resolve(
					seen.length === 1 ? redirect('https://end.test/b') : page('<head><title>Landed</title></head>'),
				);
	...
	it('refuses a redirect that points at a private address', async () => {
		...
		const work = fetchLinkPreview('https://start.test/a', directory, {
			resolve: (hostname) => Promise.resolve(hostname === 'start.test' ? ['1.1.1.1'] : ['127.0.0.1']),
			fetchImpl: ((input: URL) => {
	...
	it('gives up rather than following a redirect loop', async () => {
		const work = fetchLinkPreview('https://loop.test/a', directory, {
			resolve: () => Promise.resolve(['1.1.1.1']),
			fetchImpl: (() => Promise.resolve(redirect('https://loop.test/a'))) as unknown as typeof fetch,
		});
	...
	it('a preview whose image cannot be fetched keeps everything else', async () => {
		const preview = await fetchLinkPreview('https://img.test/a', directory, {
			...
			fetchImpl: ((input: URL) => {
				...
				return Promise.resolve(
					page('<head><title>Has image</title><meta property="og:image" content="http://cdn.internal/x.png"></head>'),
				);
			}) as unknown as typeof fetch,
		});
