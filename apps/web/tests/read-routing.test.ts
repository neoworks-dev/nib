...
describe("where a read goes", () => {
  ...
  test("a model goes to the viewer that can turn it, not to the editor", () => {
    expect(
      routeRead({ path: "art/ship.glb", attached: ["editor", "model"], ...following }),
    ).toEqual({
      kind: "model",
      path: "art/ship.glb",
    });
  ...
  test("a source file does not go to a model viewer", () => {
    expect(routeRead({ path: "src/a.ts", attached: ["model"], ...following })).toEqual({
      kind: "none",
    });
  });
  ...
  test("with nothing attached, nothing happens", () => {
    ...
    expect(routeRead({ path: "src/a.ts", attached: ["git", "terminal"], ...following })).toEqual({
      kind: "none",
    });
  });
  ...
  test("nothing opens under a user who is reading an older turn", () => {
    expect(
      routeRead({ path: "src/a.ts", attached: ["editor"], lastRouted: null, following: false }),
    ).toEqual({
      kind: "none",
    });
  });
