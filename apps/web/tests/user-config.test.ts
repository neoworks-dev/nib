...
describe("mergeUserConfig", () => {
  it("keeps settings written by other builds", () => {
    ...
    expect(merged).toEqual({
      theme: "dark",
      defaultModels: { codex: "gpt-5", "claude-code": "sonnet" },
      lastProject: null,
    });
  });
  ...
  it("keeps the stored project when the patch does not name one", () => {
    expect(mergeUserConfig({ lastProject: "/w/app" }, { defaultModels: {} }).lastProject).toBe(
      "/w/app",
    );
  });
  ...
  it("takes the project from the patch", () => {
    expect(
      mergeUserConfig({ lastProject: "/w/app" }, { lastProject: "/w/other" }).lastProject,
    ).toBe("/w/other");
  });
});
...
describe("user config file", () => {
  ...
  it("creates the file on first write and reads the value back", async () => {
    ...
    expect(JSON.parse(readFileSync(userConfigPath(), "utf8")).defaultModels).toEqual({
      "claude-code": "opus",
    });
