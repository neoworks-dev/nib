import { type EvaluableFork, Kernel } from "./kernel";
...
class Fork<C> implements EvaluableFork, ForkHandle {
  ...
  evaluate(): boolean {
    ...
    const satisfied = (this.plugin.inject ?? []).every((service) =>
      this.kernel.services.has(service),
    );
  ...
  private activate(): void {
    ...
    for (const name of this.plugin.inject ?? [])
      committed.set(name, this.kernel.services.get(name));
    try {
...
export class Context {
  ...
  on<K extends EventName>(event: K, listener: EventListener<K>): Disposer {
    const remove = this.kernel.addListener(
      event,
