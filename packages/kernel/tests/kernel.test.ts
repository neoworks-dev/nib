...
import { createContext, type Context, type Disposer, type Plugin } from '@nib-ui/kernel';
...
describe('services and events', () => {
	test('providing the same service twice throws', () => {
		const ctx = createContext();
		ctx.use(counterProvider);
		expect(() => ctx.provide('counter', { value: 2 })).toThrow('service "counter" is already provided');
	});
