...
const blockContentSchema = z.union([
	...
	z.object({ kind: z.literal('tool_use'), toolName: z.string(), toolUseId: z.string(), input: z.unknown() }),
	z.object({
	...
	z.object({ kind: z.literal('image'), mediaType: z.string(), data: z.string().optional(), url: z.string().optional() }),
...
export type EventDataMap = { [K in keyof typeof eventDataSchemas]: z.infer<(typeof eventDataSchemas)[K]> };
...
export type KnownAgentEvent = { [K in KnownEventType]: AgentEvent<K, EventDataMap[K]> }[KnownEventType];
...
export type EmittedEvent = { [K in KnownEventType]: { type: K; data: EventDataMap[K]; raw?: unknown } }[KnownEventType];
