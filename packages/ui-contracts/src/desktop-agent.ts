...
export function describeCapabilities(host: HostReport): DesktopCapabilities {
  if (!host.bridge) {
    return disabledCapabilities(
      "Desktop control needs the desktop app; a browser has no access to the screen.",
    );
  }
  if (!host.sidecar) {
  ...
  if (sidecar.focus === "unavailable") {
    notes.push(
      "The focused application cannot be identified, so per-application routing falls back to the default.",
    );
  }
  if (sidecar.pointer === "unavailable") {
    notes.push(
      "This compositor exposes no cursor position, so the overlay cannot follow the pointer.",
    );
  }
