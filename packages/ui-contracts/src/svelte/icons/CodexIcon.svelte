<script lang="ts">
	...
	// OpenAI Codex mark, from lobe-icons (MIT), inlined to avoid an icon-package dependency.
	const { size = 16, color: _color, weight: _weight, mirrored: _mirrored, ...rest }: BrandIconProps = $props();

	// The gradient is referenced by id, so every instance needs its own.
