import { blockToolInput, type BlockView } from '@nib-ui/protocol';
...
export function describeCall(block: BlockView): StepDescriptor {
	...
	switch (toolName) {
		...
		case 'TodoWrite':
			return { label: 'Plan', noun: 'update', verb: 'Updated', detail: 'the plan', subject: 'the plan' };
		case 'Task':
...
function step(label: string, noun: string, verb: string, subject: string, abbreviate: (value: string) => string): StepDescriptor {
...
export function countedNoun(count: number, noun: string): string {
	...
	const suffix = noun.endsWith('ch') || noun.endsWith('sh') || noun.endsWith('s') || noun.endsWith('x') ? 'es' : 's';
