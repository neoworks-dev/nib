...
describe('a machine with everything', () => {
	...
	test('the compositor is carried but never decides anything', () => {
		const renamed = describeCapabilities({ bridge: true, sidecarResolved: true, sidecar: { ...wayland, compositor: 'something-else' } });
...
describe('degradations', () => {
	...
	test('no layer-shell on Wayland leaves the overlay unavailable, not a floating window', () => {
		...
		expect(report.overlay).toBe('unavailable');
		expect(report.notes.some((note) => note.includes('cannot be positioned on Wayland'))).toBe(true);
	});
	...
	test('no capture strategy is explained', () => {
		const report = describeCapabilities({ bridge: true, sidecarResolved: true, sidecar: { ...wayland, capture: 'unavailable' } });
		expect(report.notes.some((note) => note.includes('neither the desktop portal nor grim'))).toBe(true);
	});

	test('no focus source explains that routing falls back', () => {
		const report = describeCapabilities({ bridge: true, sidecarResolved: true, sidecar: { ...wayland, focus: 'unavailable' } });
	...
	test('no pointer source explains that the overlay cannot follow', () => {
		const report = describeCapabilities({ bridge: true, sidecarResolved: true, sidecar: { ...wayland, pointer: 'unavailable' } });
	...
	test('the sidecar’s own notes come first, then the host’s', () => {
		const report = describeCapabilities({
			bridge: true,
			sidecarResolved: true,
			sidecar: { ...wayland, notes: ['grim was not found'], capture: 'unavailable', pointer: 'unavailable' },
		});
...
describe('anyCapability', () => {
	test('capture alone is enough to be worth offering', () => {
		expect(anyCapability(describeCapabilities({ bridge: true, sidecarResolved: false, sidecar: wayland }))).toBe(true);
	});

	test('a browser is not', () => {
		expect(anyCapability(describeCapabilities({ bridge: false, sidecarResolved: false }))).toBe(false);
	});
