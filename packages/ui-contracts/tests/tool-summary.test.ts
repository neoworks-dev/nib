...
describe('describeStep', () => {
	test('names the file a read or edit touched', () => {
		expect(describeStep(toolUse('Read', { file_path: '/repo/apps/web/README.md' }))).toBe('Read README.md');
	...
	test('quotes the command a shell call ran', () => {
		expect(describeStep(toolUse('Bash', { command: 'node --check app.js' }))).toBe('Ran node --check app.js');
	});
	...
	test('reduces a fetch to its host', () => {
		expect(describeStep(toolUse('WebFetch', { url: 'https://example.com/a/b?c=1' }))).toBe('Fetched example.com');
	});
...
describe('describeCall', () => {
	test('reads the command out of half-arrived input', () => {
		expect(describeCall(streamingCall('Bash', '{"command": "git status --por')).detail).toBe('git status --por');
		expect(describeCall(streamingCall('Bash', '{"command": "echo \\"hi\\" && ls')).detail).toBe('echo "hi" && ls');
	...
	test('survives a command clipped mid-escape', () => {
		...
		expect(describeCall(streamingCall('Bash', '{"command": "echo one\\u00')).detail).toBe('echo one');
	});

	test('names the run a call belongs to', () => {
		expect(describeCall(toolUse('Bash', { command: 'ls' }))).toMatchObject({ label: 'Terminal', noun: 'command' });
		expect(describeCall(toolUse('Grep', { pattern: 'todo' }))).toMatchObject({ label: 'Explore', noun: 'search' });
		expect(describeCall(toolUse('Read', { file_path: 'a.ts' }))).toMatchObject({ label: 'Explore', noun: 'file' });
		expect(describeCall(toolUse('Write', { file_path: 'a.ts' }))).toMatchObject({ label: 'Edit', noun: 'change' });
	});
...
describe('readFilePath', () => {
	test('a read hands back the whole path, not the name the transcript prints', () => {
		expect(readFilePath(toolUse('Read', { file_path: 'apps/web/README.md' }))).toBe('apps/web/README.md');
		expect(readFilePath(toolUse('NotebookRead', { file_path: 'notes/run.ipynb' }))).toBe('notes/run.ipynb');
	});
