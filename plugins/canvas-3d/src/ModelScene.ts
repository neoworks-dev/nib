...
import { type ModelFormat, type ModelOrbit, orbitCameraPosition } from "./model";
...
export class ModelScene {
  ...
  async load(url: string, format: ModelFormat): Promise<boolean> {
    ...
    if (box.isEmpty() || !Number.isFinite(box.min.x))
      throw new Error("the file carries no drawable geometry");
  ...
  render(orbit: ModelOrbit): void {
    ...
    const changed =
      sharedSize.width !== this.width ||
      sharedSize.height !== this.height ||
      sharedSize.resolution !== this.resolution;
    if (changed) {
