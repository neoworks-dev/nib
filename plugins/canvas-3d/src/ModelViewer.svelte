<script lang="ts">
  ...
  function draw() {
    ...
    active.resize(
      width,
      height,
      Math.min(MAX_RESOLUTION, Math.max(1, globalThis.devicePixelRatio || 1)),
    );
...
<div aria-label="3D model viewer" class="relative flex h-full min-h-0 flex-col">
  ...
  {#if status !== "ready"}
    <p
      class="pointer-events-none absolute inset-0 flex items-center justify-center text-xs text-dim"
    >
      {#if status === "empty"}
  ...
  <p class="border-t border-line px-2.5 py-1 text-2xs text-faint">
    Drag to turn · shift-drag or wheel to zoom
  </p>
</div>
