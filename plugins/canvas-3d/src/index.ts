...
import { storeWorkspaceAsset, type UploadedModel, uploadModel } from "./client";
 ...
 */
export async function placeWorkspaceModel(
  placement: WorkspaceModelPlacement,
): Promise<string | null> {
...
export const canvas3dPlugin: Plugin = {
  ...
  apply(ctx) {
    ...
    ctx.effect(() =>
      canvas.registerPasteHandler({ order: 5, handle: (payload, at) => place(payload.files, at) }),
    );
    ctx.effect(() =>
      canvas.registerDropHandler({ order: 5, handle: (payload, at) => place(payload.files, at) }),
    );
