 ...
 */
export function orbitAfterDrag(
  from: ModelOrbit,
  deltaX: number,
  deltaY: number,
  dolly: boolean,
  ...
  if (dolly) {
    return {
      yaw,
      pitch: from.pitch,
      distance: clampDistance(from.distance + deltaY * DISTANCE_PER_PIXEL),
    };
  }
...
function isObj(bytes: Uint8Array): boolean {
  ...
  const head = String.fromCharCode(
    ...bytes.subarray(start, start + Math.min(bytes.byteLength - start, 4096)),
  );
  ...
  return {
    ...
    yaw:
      typeof candidate.yaw === "number" && Number.isFinite(candidate.yaw)
        ? candidate.yaw
        : MODEL_DEFAULT_ORBIT.yaw,
    pitch: clampPitch(
      typeof candidate.pitch === "number" ? candidate.pitch : MODEL_DEFAULT_ORBIT.pitch,
    ),
    distance: clampDistance(
      typeof candidate.distance === "number" ? candidate.distance : MODEL_DEFAULT_ORBIT.distance,
    ),
