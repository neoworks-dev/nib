...
import type { CanvasRegistry, CanvasPastePayload, Point } from '@nib-ui/ui-contracts';
import { assetUrl, measureMedia, storeWorkspaceAsset, uploadMedia } from './client';
import { mediaKind } from './MediaRenderer';
import { isMediaFile, mediaMime, mediaTypeFor, mediaTypeForName, parseMedia, type MediaObject } from './media';
 ...
 */
export async function placeWorkspaceMedia(placement: WorkspaceMediaPlacement): Promise<string | null> {
	...
	const size = stored.ok ? await measureMedia(await stored.blob(), mediaType) : { width: 320, height: 320 };
...
export const canvasMediaPlugin: Plugin = {
	...
	apply(ctx) {
		...
		ctx.effect(() => canvas.registerDropHandler({ order: 10, handle: (payload, at) => place(payload.files, at) }));
