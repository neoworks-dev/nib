<script lang="ts">
	...
	import { parseWorkspaceFileRefs, workspaceFileTransferType, type PaneProps } from '@nib-ui/ui-contracts';
	...
	const boardComposer = $derived.by((): PendingComposer => {
		...
		return {
			...
			start: async (text) => {
				...
				const at = boardPoint();
				await canvasState.startWorkstream(text, { x: at.x - CARD_WIDTH / 2, y: at.y - CARD_MIN_HEIGHT });
			},
		};
	});
	...
	function zoomBy(factor: number) {
		...
		if (!rect) return;
		registry.setCamera(
			zoomAt(rect.width / 2, rect.height / 2, clampZoom(registry.camera.zoom * factor), registry.camera),
		);
	}
...
<div bind:clientWidth={boardWidth} bind:clientHeight={boardHeight} class="relative h-full w-full overflow-hidden bg-canvas">
	...
	{#if !opened}
		<p class="pointer-events-none absolute inset-0 z-raised flex items-center justify-center text-sm text-dim">
	...
	{#if canvasState.error}
		<p class="absolute top-2 left-1/2 z-raised -translate-x-1/2 rounded-lg border border-red/40 bg-red-soft px-2 py-1 text-2xs text-red">
			{canvasState.error}
		</p>
	{/if}
	...
	{#if menu}
		...
		<!-- svelte-ignore a11y_no_static_element_interactions -->
		<div class="absolute inset-0 z-overlay" onclick={() => registry.closeMenu()} oncontextmenu={(event) => event.preventDefault()}>
			<div
