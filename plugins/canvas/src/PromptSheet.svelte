...
{#if sheet}
  ...
  >
    <div
      class="flex w-[520px] max-w-full flex-col gap-2 rounded-2xl border border-line bg-elevated px-4 py-3.5"
    >
      ...
      {#if anchor}
        <p class="truncate rounded-md bg-raised px-2 py-1 text-2xs text-muted">
          from “{anchorTitle}”
        </p>
      {/if}
      ...
      <div class="flex items-center gap-2">
        ...
        <span class="ml-auto flex items-center gap-2">
          ...
          {:else}
            <Button size="sm" variant="ghost" onclick={() => canvasState.closeSheet()}
              >Cancel</Button
            >
          {/if}
          <Button
