...
export function updateObject(objects: CanvasObject[], id: string, patch: Partial<CanvasObject>): CanvasObject[] {
	let changed = false;
...
export function withCascade(objects: CanvasObject[], ids: string[]): string[] {
	...
	for (const object of objects) {
		...
		if (removing.has(object.fromId as string) || removing.has(object.toId as string)) removing.add(object.id);
	}
	return [...removing];
}
 ...
 */
export function rebaseObjects(server: CanvasObject[], local: CanvasObject[], removedIds: string[]): CanvasObject[] {
