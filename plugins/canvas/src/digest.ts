 ...
 */
export function transcriptDigest(turns: DigestEntry[], budget = FULL_CONTEXT_BUDGET): DigestEntry[] {
...
export function referenceSeed(references: string[], instruction: string): string {
	...
	return ['Working from what is attached or linked:', '', ...references.map((entry) => `- ${entry}`), '', body].join(
		'\n',
	);
}
