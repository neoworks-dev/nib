...
export function isConnectable(renderer: CanvasObjectRenderer | undefined): renderer is CanvasObjectRenderer &
	ConnectableRenderer {
...
export function drawPorts(
	...
	for (const port of PORTS) {
		...
		graphics.roundRect(center.x - half, center.y - half, half * 2, half * 2, radius);
		graphics.stroke({ width: 1 / scale, color: hot ? paint.hot : paint.ring, alpha: opacity * 0.9 });
