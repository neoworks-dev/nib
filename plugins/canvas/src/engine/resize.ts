...
export function isResizable(renderer: CanvasObjectRenderer | undefined): renderer is CanvasObjectRenderer &
	ResizableRenderer {
...
export function resizedRect(
	...
	const height = Math.round(
		freeAspect ? Math.max(minimum, from.height + growY) : Math.max(minimum, width * (from.height / from.width)),
	);
...
export function isPressable(renderer: CanvasObjectRenderer | undefined): renderer is CanvasObjectRenderer &
	PressableRenderer {
