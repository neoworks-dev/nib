...
type State =
	...
	| { kind: 'dragging'; screen: Point; origins: { id: string; x: number; y: number }[]; history: Disposer }
...
export class SelectTool implements CanvasTool {
	...
	onPointerDown(event: CanvasPointerEvent): boolean {
		...
		if (!engine.selection.includes(hitId)) engine.select(additive ? [...engine.selection, hitId] : [hitId]);
	...
	onPointerMove(event: CanvasPointerEvent): void {
		...
		if (this.state.kind === 'spawning') {
			const overId = engine.hitTest(event.screen.x, event.screen.y);
			const travelled = Math.hypot(event.screen.x - this.state.screen.x, event.screen.y - this.state.screen.y);
			this.state = {
				...this.state,
				pointer: event.world,
			...
			if (this.state.moved) {
				this.setCursor('crosshair');
				this.drawLinks(this.spawnAnchors(this.state.sourceIds, event.world), event.world, this.state.overId);
			}
			return;
		}
		...
		if (this.state.kind === 'pending') {
			const moved = Math.hypot(event.screen.x - this.state.screen.x, event.screen.y - this.state.screen.y);
			if (moved <= DRAG_THRESHOLD) return;

			const ids = engine.selection.includes(this.state.targetId) ? engine.selection : [this.state.targetId];
			const origins = ids
			...
			this.state = { kind: 'dragging', screen: this.state.screen, origins, history: engine.beginHistory() };
		}
		...
		if (this.state.kind === 'rubber') {
			...
			this.drawBand();
			engine.select(engine.objectsIntersecting(rectFromCorners(this.state.anchor, this.state.corner)));
			return;
		}
	...
	onPointerUp(event: CanvasPointerEvent): void {
		...
		switch (state.kind) {
			...
			case 'connecting': {
				...
				engine?.connect(state.fromId, targetId && targetId !== state.fromId ? targetId : null, event.world);
				return;
			}
			case 'resizing': {
