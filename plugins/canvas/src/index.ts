...
export const canvasPlugin: Plugin = {
	...
	apply(ctx) {
		...
		ctx.effect(() =>
			registry.registerKind(
				workstreamKind({
					...
					respondToPermission: (sessionId, allow) => canvasState.respondToFirstPermission(sessionId, allow),
		...
		ctx.effect(() =>
			registry.registerContextMenu({
				items: (target, at): CanvasMenuItem[] => {
					...
					if (target.kind === 'edge') {
						return [
							{ kind: 'action', id: 'canvas.deleteEdge', label: 'Delete link', run: () => registry.removeObjects([target.id]) },
						];
					}
					...
					return [
						{ kind: 'action', id: 'canvas.open', label: 'Open transcript', run: () => canvasState.open(target.id) },
						{
							kind: 'action',
							id: 'canvas.branch',
						...
						{ kind: 'action', id: 'canvas.delete', label: 'Remove from board', run: () => registry.removeObjects([target.id]) },
					];
				},
			}),
		);
		...
		ctx.effect(() => panes.register({ id: paneId, kind: 'canvas', title: 'Canvas', icon: GraphIcon, component: CanvasPane }));
		...
		ctx.effect(() =>
			panes.register({ id: chatPaneId, kind: 'chat', title: 'Chat', icon: ChatCenteredDotsIcon, component: ChatPane }),
		);
		...
		ctx.effect(() => {
			const onKeydown = (event: KeyboardEvent) => {
				if (!(event.ctrlKey || event.metaKey) || !event.shiftKey || event.key.toLowerCase() !== 'b') return;
...
export { chatPaneId, paramsForWorkstream, type ChatPaneParams, type ChatPaneTarget } from './chat-pane';
export { branchSeed, digestOf, joinLabel, joinSeed, referenceSeed, type DigestEntry } from './digest';
...
export { legacyStorageKey, migrateDocs, parseLegacyDocs, type LegacyDoc, type MigratedBoard } from './migrate';
...
export { bakeRuns, measureRuns, resolutionForZoom, TextTextureCache, type TextRun } from './engine/utils/textTexture';
export { pointInRect, rectFromCorners, rectsIntersect, unionRects } from './engine/utils/geometry';
export { boardTheme, statusColor, statusWord, themeRevision, type BoardTheme } from './theme';
