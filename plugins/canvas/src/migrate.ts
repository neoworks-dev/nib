...
import { CARD_WIDTH, type AnnotationObject, type EdgeObject, type WorkstreamObject } from './workstream';
 ...
 */
export function migrateDocs(docs: Record<string, LegacyDoc>, cwdFor: (sessionId: string) => string | null): MigratedBoard[] {
...
function migrateDoc(doc: LegacyDoc, alreadyPlaced: number): CanvasObject[] {
	...
	const place = (id: string, sessionId: string | null, goal: string, nodeIds: string[]): WorkstreamObject => {
	...
	for (const [sessionId, workstream] of workstreamBySession) {
		for (const key of positionKeysFor(doc.positions, sessionId)) workstreamByNode.set(key, workstream);
	}
...
function edge(fromId: string, toId: string, label: string): EdgeObject {
	return { kind: 'edge', id: `edge:${fromId}->${toId}:${label}`, fromId, toId, label, direction: 'forward' };
}
...
function carriedPosition(
	...
	return points.reduce((best, point) => (point.y < best.y || (point.y === best.y && point.x < best.x) ? point : best));
}
