...
import { curveBetween, distanceToCurve, pointOnCurve, type Curve } from '../engine/utils/geometry';
import type { BoardTheme } from '../theme';
import { parseEdge, type EdgeObject } from '../workstream';
...
export class EdgeRenderer extends ObjectRenderer<EdgeObject> {
	...
	sync(data: EdgeObject, selection: string[]): void {
		...
		line.bezierCurveTo(curve.control1.x, curve.control1.y, curve.control2.x, curve.control2.y, curve.end.x, curve.end.y);
	...
	private arrowHead(from: Point, to: Point, color: number, alpha: number): void {
		...
		this.line.lineTo(to.x - ARROW * Math.cos(angle - Math.PI / 6), to.y - ARROW * Math.sin(angle - Math.PI / 6));
		this.line.lineTo(to.x - ARROW * Math.cos(angle + Math.PI / 6), to.y - ARROW * Math.sin(angle + Math.PI / 6));
		this.line.closePath();
