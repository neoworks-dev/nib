...
import { type BoardTheme, statusColor, statusWord, themeRevision } from "../theme";
import {
  CARD_MIN_HEIGHT,
  CARD_MIN_WIDTH,
  CARD_WIDTH,
  ...
  implements ResizableRenderer, ConnectableRenderer
{
  ...
  private redraw(): void {
    ...
    const meta =
      metaLine.length > 0
        ? this.bake(
            `meta:${view.id}`,
            [{ text: metaLine, size: 10, color: theme.faint, maxLines: 1 }],
            contentWidth,
          )
        : null;
  ...
  private drawFrame(accent: number, theme: BoardTheme): void {
    ...
    const label = this.bake(
      ...
      [
        {
          text: statusWord(view?.status ?? "idle").toUpperCase(),
          size: 9,
          weight: 600,
          color: theme.faint,
          maxLines: 1,
          ...
          [
            {
              ...
              size: 10,
              weight: 600,
              color: text === "Allow" ? "#4ade80" : "#f87171",
              maxLines: 1,
            },
          ],
          rect.width,
