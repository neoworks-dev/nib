...
export class CanvasRegistryStore implements CanvasRegistry, EngineHost {
	...
	registerKind(kind: CanvasObjectKind): Disposer {
		if (this.kinds.has(kind.kind)) throw new Error(`canvas kind "${kind.kind}" is already registered`);
	...
	select(ids: string[]): void {
		if (ids.length === this.selection.length && ids.every((id, index) => this.selection[index] === id)) return;
		this.selection = ids;
	}
