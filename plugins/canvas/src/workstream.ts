...
export function parseWorkstream(raw: unknown): WorkstreamObject | null {
	...
	return {
		...
		...(typeof candidate.permissionMode === 'string' && { permissionMode: candidate.permissionMode }),
...
export function parseEdge(raw: unknown): EdgeObject | null {
	...
	return {
		...
		direction:
			direction === 'backward' || direction === 'both' || direction === 'none' ? direction : 'forward',
 ...
 */
export function workstreamAt(objects: CanvasObject[], point: { x: number; y: number }): WorkstreamObject | null {
	for (let index = objects.length - 1; index >= 0; index -= 1) {
		...
		const inside =
			point.x >= object.x && point.x <= object.x + width && point.y >= object.y && point.y <= object.y + height;
 ...
 */
export function freeSlot(objects: CanvasObject[], near: { x: number; y: number }): { x: number; y: number } {
	const taken = objects.filter(isWorkstream);
	let { x, y } = near;
	let guard = 0;

	while (guard < 200 && taken.some((object) => Math.abs(object.x - x) < 24 && Math.abs(object.y - y) < 24)) {
		x += 32;
		y += 32;
		guard += 1;
	}
