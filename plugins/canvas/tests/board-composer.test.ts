...
import type { BoardDoc, CreateSessionInput, PaneRegistry, SessionsService, TransportService } from '@nib-ui/ui-contracts';
...
describe('the board composer', () => {
	...
	test('another harness drops the picks made against the one before it', () => {
		...
		expect(canvasState.boardSettings).toMatchObject({ harnessId: 'codex', model: null, effort: null });
	});
	...
	test('sending puts a card on the board and starts it in one move', async () => {
		...
		expect(cards()[0]).toMatchObject({ id: id!, goal: 'port the engine', x: 40, y: 60, sessionId: 's1' });
	...
	test('the transcript pane comes up with the card, and follows it onto its session', async () => {
		...
		expect(panes.reparamed).toEqual([{ instanceId: 'i1', params: { workstreamId: id!, sessionId: 's1' } }]);
	});
	...
	test('a join is a card with its sources named on it, not a prompt to fill in first', async () => {
		...
		expect(panes.opened.at(-1)).toEqual({ paneId: chatPaneId, params: { workstreamId: joined!.id } });
