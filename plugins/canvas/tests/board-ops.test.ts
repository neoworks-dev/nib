...
describe("updateObject", () => {
  ...
  test("a patch may move an object but never retype it", () => {
    const objects = updateObject([workstream("a")], "a", {
      kind: "note",
      id: "z",
      x: 5,
...
describe("rebaseObjects", () => {
  test("another window's objects survive this window's write", () => {
    ...
    expect(rebaseObjects(server, local, []).map((object) => object.id)).toEqual([
      "a",
      "theirs",
      "mine",
    ]);
  ...
  test("a kind this build cannot render is carried through untouched", () => {
    const exotic: CanvasObject = {
      kind: "stroke",
      id: "ink",
      points: [{ x: 1, y: 2, pressure: 0.4 }],
      color: "#fff",
      width: 2,
    };
