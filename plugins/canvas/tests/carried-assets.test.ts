...
import { isWorkstream, type EdgeObject, type WorkstreamObject } from '../src/workstream';
	...
	readonly harnesses = [
		{
			...
			capabilities: { interrupt: true, permissionModes: ['default'], resume: true, fork: true, slashCommands: true, models: true },
...
const picture: CanvasObject = { kind: 'media', id: 'media:1', x: 100, y: 100, assetId: 'abc.png', name: 'shot.png' };
		...
		contextFor: (object) =>
			object.kind === 'media'
				? {
						...
						attachments: [{ assetId: String(object.assetId), mime: 'image/png', name: String(object.name) }],
					}
				: null,
	});
}
...
describe('starting a workstream from an object', () => {
	...
	test('an object no plugin speaks for still names itself and can be started from', () => {
		canvasState.board.addObject({ kind: 'sticky', id: 'sticky:1', x: 0, y: 0, title: 'rewrite the intro' });
...
describe('the files a workstream carries', () => {
	...
	test('a picture linked to a running task waits for its next prompt', () => {
		...
		canvasState.board.addObject({ kind: 'media', id: 'media:2', x: 0, y: 0, assetId: 'def.png', name: 'other.png' });
	...
	test('a node with nothing to attach travels as a line in the prompt', async () => {
		canvasState.registry.registerContextProvider({
			contextFor: (object) =>
				object.kind === 'bookmark' ? { label: 'Pixi docs', text: 'Pixi docs — https://pixijs.com' } : null,
		});
		canvasState.board.addObject({ kind: 'bookmark', id: 'bookmark:1', x: 0, y: 0, url: 'https://pixijs.com' });
