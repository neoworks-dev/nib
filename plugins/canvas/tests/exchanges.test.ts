...
describe('toExchanges', () => {
	test('opens an exchange per prompt and folds the answer into it', () => {
		...
		expect(exchanges.map((exchange) => exchange.id)).toEqual([exchangeId('s1', 'm1'), exchangeId('s1', 'm3')]);
		...
		const exchanges = toExchanges(
			session('s1', [
				...
				message('m2', 'assistant', [
					...
					toolUse('t2', 'Edit', { file_path: '/a/b.ts', old_string: 'one\ntwo', new_string: 'one\nthree' }),
	...
	test('a transcript opening with harness output still gets an exchange', () => {
		const exchanges = toExchanges(session('s1', [message('m1', 'assistant', [text('b1', 'resumed')])]));
