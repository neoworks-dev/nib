...
import { migrateDocs, parseLegacyDocs, type LegacyDoc } from '../src/migrate';
import type { EdgeObject, WorkstreamObject } from '../src/workstream';

function doc(overrides: Partial<LegacyDoc> = {}): LegacyDoc {
	return { rootSessionId: 'root', branches: [], joins: [], positions: {}, annotations: [], ...overrides };
}
...
describe('migrateDocs', () => {
	...
	test('a thread started on empty board space has nothing to link to', () => {
		const boards = migrateDocs({ root: doc({ branches: [{ sessionId: 'side', fromNodeId: null }] }) }, inRepo);
	...
	test('a join keeps one edge per source and stays unlaunched when it never ran', () => {
		const boards = migrateDocs(
			{
				root: doc({
					...
					joins: [{ id: 'join:1', label: 'both', sourceIds: ['root#1', 'side#1'], sessionId: null }],
				}),
			},
			inRepo,
		);
		...
		expect(edges(boards[0]!.objects).filter((link) => link.label === 'join').map((link) => link.fromId).sort()).toEqual([
			'workstream:root',
			'workstream:side',
		]);
	});
	...
	test('a launched join carries its session onto the card', () => {
		const boards = migrateDocs(
			{ root: doc({ joins: [{ id: 'join:1', label: 'both', sourceIds: ['root#1'], sessionId: 'merged' }] }) },
			inRepo,
		);

		expect(workstreams(boards[0]!.objects).find((card) => card.id === 'workstream:join:1')!.sessionId).toBe('merged');
	});
	...
	test('pinned quotes survive with the turn they were highlighted from', () => {
		const boards = migrateDocs(
			{
				root: doc({
					annotations: [{ id: 'q1', sessionId: 'root', messageId: 'm4', text: 'the bit', note: 'why?' }],
				}),
			},
			inRepo,
		);
...
describe('parseLegacyDocs', () => {
	...
	test('missing arrays and a missing annotations field are tolerated', () => {
		...
		expect(parsed.root).toEqual({ rootSessionId: 'root', branches: [], joins: [], positions: {}, annotations: [] });
	});
