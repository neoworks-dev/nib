...
const bookmark: CanvasObject = { kind: 'bookmark', id: 'bookmark:1', x: 0, y: 0, url: 'https://example.com' };
...
describe('planSpawn', () => {
	...
	test('branches when the source has no forkable conversation', () => {
		expect(planSpawn([workstream('a', 'session-a')], () => null)).toEqual({ kind: 'branch', sourceId: 'a' });
		expect(planSpawn([workstream('a')], () => 'native-a')).toEqual({ kind: 'branch', sourceId: 'a' });
	});
	...
	test('with no workstream, the objects are what the task is started from', () => {
		expect(planSpawn([bookmark], () => 'native-a')).toEqual({ kind: 'assets', sourceIds: ['bookmark:1'] });
	});
