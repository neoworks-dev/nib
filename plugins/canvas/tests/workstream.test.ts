...
describe("parseWorkstream", () => {
  ...
  test("an older board without a goal still loads", () => {
    expect(parseWorkstream({ kind: "workstream", id: "w1", x: 4, y: 5 })).toEqual(
      card({ x: 4, y: 5 }),
    );
  });
  ...
  test("the picks a workstream is to start with survive a round trip", () => {
    ...
    expect(parseWorkstream(stored)).toEqual(
      card({ harnessId: "codex", model: "gpt-5", permissionMode: "plan", effort: "high" }),
    );
  });
...
describe("launchSettings", () => {
  ...
  test("the user pick wins over the harness default", () => {
    expect(
      launchSettings(card({ model: "opus", permissionMode: "plan", effort: "high" }), [harness()]),
    ).toEqual({
      harnessId: "claude-code",
      model: "opus",
      permissionMode: "plan",
  ...
  test("nothing is registered, so there is nothing to start with", () => {
    expect(launchSettings(card(), [])).toEqual({
      harnessId: null,
      model: null,
      permissionMode: null,
      effort: null,
...
describe("launchOptions", () => {
  test("carries only what was actually chosen", () => {
    expect(
      launchOptions({ harnessId: "codex", model: "gpt-5", permissionMode: null, effort: "high" }),
    ).toEqual({
      model: "gpt-5",
      effort: "high",
    });
  ...
  test("the harness is not an option: it selects the adapter, it is not passed to it", () => {
    expect(
      launchOptions({ harnessId: "codex", model: null, permissionMode: null, effort: null }),
    ).toEqual({});
  });
});
...
describe("parseEdge", () => {
  test("an unknown direction falls back to a plain arrow", () => {
    expect(
      parseEdge({ kind: "edge", id: "e", fromId: "a", toId: "b", direction: "sideways" }),
    ).toEqual({
      kind: "edge",
      id: "e",
      fromId: "a",
...
describe("workstreamView", () => {
  ...
  test("the title comes from the live session, never from the board", () => {
    const view = workstreamView(
      card({ sessionId: "s", goal: "stale goal" }),
      chat("s", ["do the thing"], { title: "Do the thing" }),
    );
  ...
  test("a detached card reports what it was started with, never a harness default", () => {
    ...
    expect(workstreamView(card({ sessionId: "s", model: "opus" }), null, [], "sonnet").model).toBe(
      "opus",
    );
  });
  ...
  test("a live session that has not reported a model falls back to the launch record", () => {
    const live = chat("s", ["go"]);
    expect(workstreamView(card({ sessionId: "s", model: "opus" }), live, [], "sonnet").model).toBe(
      "opus",
    );

    expect(
      workstreamView(
        card({ sessionId: "s", model: "opus" }),
        { ...live, model: "haiku" },
        [],
        "sonnet",
      ).model,
