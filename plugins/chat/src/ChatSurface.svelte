<script lang="ts">
	...
	$effect(() => {
		...
		feed.querySelector(`[data-message="${CSS.escape(scrollToMessageId)}"]`)?.scrollIntoView({ block: 'start' });
		follow = false;
	});
	...
	function readSelection() {
		...
		if (!feed || !anchor || !feedFrame || !messageId || !feed.contains(anchor) || text.trim().length === 0) {
			offer = null;
			return;
		}
	...
	function stageNote() {
		if (!composing) return;
		const annotation = createAnnotation(session.sessionId, composing.messageId, composing.text, note);
		cancelNote();
	...
	function clearAnnotations() {
		...
		for (const annotationId of annotations.map((entry) => entry.id)) onAnnotationRemove?.(annotationId);
	}
</script>
...
<div class="flex h-full min-h-0 flex-col">
	<div bind:this={feedFrame} class="relative min-h-0 flex-1">
		...
		{#if composing}
			...
			>
				<div class="flex flex-col gap-1.5 rounded-xl border border-line bg-elevated px-2.5 py-2 shadow-lg">
					<p class="line-clamp-2 border-l-2 border-line-strong pl-2 text-2xs leading-relaxed text-dim">
						{composing.text}
					</p>
					<input
						bind:this={noteInput}
		...
		<div class="flex flex-wrap items-center gap-1.5 px-8 pb-2">
			{#each annotations as annotation (annotation.id)}
				...
				>
					...
					<span class="min-w-0 flex-1 truncate" title={annotation.text}>{annotation.note ?? annotation.text}</span>
					<button
						type="button"
	...
	<Composer {session} {annotations} {attachments} {onAttachmentsSent} onAnnotationsSent={clearAnnotations} />
</div>
