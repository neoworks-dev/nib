...
import { composeAnnotatedMessage, createAnnotation, type StagedAnnotation } from '../src/annotations';
...
describe('createAnnotation', () => {
	test('carries the session, the anchor turn and the trimmed selection', () => {
		...
		expect(staged).toMatchObject({ sessionId: 'session-1', messageId: 'message-7', text: 'the failing assertion' });
	...
	test('a written comment is trimmed onto the annotation', () => {
		expect(createAnnotation('session-1', 'message-1', 'the loop', '  why retry here?  ')).toMatchObject({
			text: 'the loop',
			note: 'why retry here?',
		});
	});
...
describe('composeAnnotatedMessage', () => {
	...
	test('a note follows its quote across a blank line, ahead of the prompt', () => {
		expect(composeAnnotatedMessage([annotation('the retry loop', 'why is this needed?')], 'and the timeout?')).toBe(
			'> the retry loop\n\nwhy is this needed?\n\nand the timeout?',
		);
	});

	test('quotes with and without notes keep their pairing', () => {
		expect(composeAnnotatedMessage([annotation('one', 'first note'), annotation('two')], 'compare')).toBe(
			'> one\n\nfirst note\n\n> two\n\ncompare',
		);
	});

	test('an annotated quote alone is the whole message', () => {
		expect(composeAnnotatedMessage([annotation('this line', 'explain it')], '')).toBe('> this line\n\nexplain it');
	});
