...
const conversation = () =>
	session(
		's1',
		[message('m1', 'user', [text('b1', 'add a button')]), message('m2', 'assistant', [text('b2', 'done')])],
...
describe('planHarnessSwitch', () => {
	test('the plan is a new session in the same directory, seeded with the transcript', () => {
		...
		expect(plan).toMatchObject({ fromSessionId: 's1', fromHarnessId: 'claude-code', toHarnessId: 'codex', cwd: '/repo' });
	...
	test('a session with no working directory has nowhere to be recreated', () => {
		expect(planHarnessSwitch(session('s1', [], { harnessId: 'claude-code', cwd: null }), 'codex')).toBeNull();
	});

	test('a conversation with nothing in it still switches, on an empty-handed seed', () => {
		const plan = planHarnessSwitch(session('s1', [], { harnessId: 'claude-code', cwd: '/repo' }), 'codex');
...
describe('describeHarnessSwitch', () => {
	test('the confirmation says the transcript is re-sent and charged for', () => {
		...
		expect(confirmation.body).toContain('replays the entire transcript into its first prompt as plain text');
	...
	test('one turn is not counted as turns', () => {
		const plan = planHarnessSwitch(
			session('s1', [message('m1', 'user', [text('b1', 'hello')])], { harnessId: 'claude-code', cwd: '/repo' }),
			'codex',
		)!;
