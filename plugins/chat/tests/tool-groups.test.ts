...
describe('groupTurnBlocks', () => {
	...
	test('prose between calls breaks the run', () => {
		expect(groups([toolUse('b1', 'Bash', { command: 'ls' }), prose('t1'), toolUse('b2', 'Bash', { command: 'pwd' })]))
			.toHaveLength(2);
	});
...
describe('groupCount', () => {
	test('pluralizes the run noun', () => {
		const [commands] = groups([toolUse('b1', 'Bash', { command: 'ls' }), toolUse('b2', 'Bash', { command: 'pwd' })]);
		const [searches] = groups([toolUse('g1', 'Grep', { pattern: 'a' }), toolUse('g2', 'Glob', { pattern: 'b' })]);
