<script lang="ts">
  ...
  function advance(answers: Record<string, string[]>) {
    ...
    if (parsed && isAnswerComplete(parsed, answers))
...
<Card surface="raised" padding="md" class="border border-violet">
  <div class="flex flex-col gap-4">
    <header class="flex items-center gap-2">
      ...
      <span class="text-2xs tracking-caps uppercase text-violet"
        >{question?.header ?? "Question"}</span
      >
    ...
    {:else}
      <p class="text-xs text-dim">
        This question has no options the UI can render; allow or deny the raw call.
      </p>
    {/if}

    <footer class="flex items-center gap-2">
      {#if page > 0}
        <Button size="sm" variant="ghost" icon={ArrowLeftIcon} onclick={() => (page -= 1)}
          >Back</Button
        >
      {/if}
      ...
      {:else if questions.length > 1}
        <Button size="sm" disabled={!complete} onclick={() => advance(selections)}
