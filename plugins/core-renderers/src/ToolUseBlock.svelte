<script lang="ts">
	...
	const outputText = $derived(typeof output === 'string' ? output : output ? JSON.stringify(output, null, 2) : '');
	const failed = $derived(result?.content?.kind === 'tool_result' && (result.content as { isError?: boolean }).isError);
</script>
