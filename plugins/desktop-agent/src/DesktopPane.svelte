<script lang="ts">
	...
	function rows(report: DesktopCapabilities) {
		return [
			...
			{ title: 'Overlay', subtitle: 'Drawing highlights above other windows.', value: report.overlay },
			{ title: 'Focused application', subtitle: 'Which application a capture belongs to.', value: report.focus },
			{ title: 'Pointer', subtitle: 'Where the cursor is, for an overlay that follows it.', value: report.pointer },
		];
	}
...
<div class="flex h-full min-h-0 flex-col">
	...
	<div class="min-h-0 flex-1 space-y-5 overflow-y-auto p-4">
			...
			<section class="space-y-2">
				...
				<Card padding="sm" class="space-y-2">
					<p class="text-xs text-dim">
						Capture {describeRequest(request)}{focusedName ? `, with ${focusedName} in front` : ''}? The picture goes on
						the board and is not sent anywhere until you send it.
					</p>
					<div class="flex items-center gap-3">
						<Button size="sm" onclick={() => desktopState.resolvePending(true)}>Capture</Button>
						<Button size="sm" variant="ghost" onclick={() => desktopState.resolvePending(false)}>Cancel</Button>
					</div>
				</Card>
			</section>
		{/if}
		...
		<section class="space-y-2">
			...
			<Card padding="sm" class="space-y-2">
				<ListRow
					...
					subtitle={desktopState.focused?.title ?? 'The compositor did not name the focused window.'}
				>
					{#snippet trailing()}
			...
			<div class="flex flex-wrap items-center gap-3 px-1">
				...
				<Button
					size="sm"
					variant="ghost"
					disabled={!desktopState.canCapture || desktopState.busy || capabilities?.focus === 'unavailable'}
				...
				{:else}
					<Button size="sm" variant="ghost" disabled={!desktopState.canCapture} onclick={() => desktopState.arm(5)}>
						Arm for 5 min
					</Button>
				{/if}
			</div>
			...
			{:else}
				<p class="px-1 text-2xs text-faint">
					A capture lands on the board. From there: <strong>Find elements</strong> to locate the controls in it,
					<strong>Show on screen</strong> to draw them on the desktop, <strong>Send to harness…</strong> to ask a model
					about it. Nothing leaves this machine until you send it.
				</p>
			{/if}
		</section>
		...
		{#if captures.length > 0}
			<section class="space-y-2">
				...
				{#each captures as capture (capture.id)}
					<Card padding="sm" class="space-y-2">
						<ListRow
							title={applicationName(capture.regions.application ?? null) ?? 'Unidentified application'}
						...
						<div class="flex flex-wrap items-center gap-2">
							<Button size="sm" variant="ghost" disabled={desktopState.busy} onclick={() => desktopState.detect(capture.id)}>
								Find elements
							</Button>
							{#if desktopState.overlaid === capture.id}
								<Button size="sm" variant="ghost" onclick={() => desktopState.hideOverlay()}>Hide overlay</Button>
							{:else}
								<Button size="sm" variant="ghost" disabled={blocker !== null} onclick={() => desktopState.showOverlay(capture.id)}>
									Show on screen
								</Button>
							{/if}
							<Button size="sm" variant="ghost" onclick={() => openSend(capture.id)}>Send to harness…</Button>
						</div>
						{#if blocker}
						...
						{#if sending === capture.id}
							<div class="space-y-2 border-t border-line pt-2">
								<p class="text-2xs text-faint">
									This is the last point of refusal. The picture below, these elements and this text are what leaves the
									machine.
								</p>
								<textarea
									bind:value={prompt}
									...
									class="w-full rounded border border-line bg-raised p-2 text-xs"
								></textarea>
		...
		<section class="space-y-2">
			...
			<Card padding="sm" class="space-y-2">
				<ListRow title="nib-overlay" subtitle={sidecar.detail ?? 'The native process that talks to the compositor.'}>
					{#snippet trailing()}
			...
			<div class="flex items-center gap-3 px-1">
				<Button size="sm" variant="ghost" disabled={!desktopState.available || desktopState.busy} onclick={() => desktopState.start()}>
