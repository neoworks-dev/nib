<script lang="ts">
	...
	import { MATCH_LEVELS, type CapturePolicy, type RoutingRule } from './routing';
	...
	function describeMatch(rule: RoutingRule): string {
		const parts = MATCH_LEVELS.flatMap((level) => {
			if (level === 'desktopEntry' && rule.match.desktopEntry) return [`desktop entry ${rule.match.desktopEntry}`];
	...
	async function update(index: number, patch: Partial<RoutingRule>): Promise<void> {
		const applications = config.applications.map((rule, position) => (position === index ? { ...rule, ...patch } : rule));
	...
	async function addForFocused(policy: CapturePolicy): Promise<void> {
		...
		const rule: RoutingRule = {
			match: {
				...
				...(!focused.desktopEntry && !focused.appId && focused.atspiName && { atspiName: focused.atspiName }),
			},
			policy,
		};
	...
	async function setDefault(policy: string | string[]): Promise<void> {
		...
		if (chosen) await desktopState.updateRouting({ ...config, default: { ...config.default, policy: chosen } });
	}
</script>
...
<section class="space-y-2">
	...
	<Card padding="sm" class="space-y-2">
		<ListRow title="Unknown applications" subtitle="What happens for an application with no rule of its own.">
			{#snippet trailing()}
				<div class="w-32">
		...
		<ListRow
			...
			subtitle={decision.matchedBy ? `Matched on ${decision.matchedBy}.` : 'No rule matches; the default applies.'}
		>
			{#snippet trailing()}
	...
	{#if focused}
		<div class="flex items-center gap-3 px-1">
			<Button size="sm" variant="ghost" onclick={() => addForFocused('allow')}>Always allow this app</Button>
			<Button size="sm" variant="ghost" onclick={() => addForFocused('deny')}>Never capture this app</Button>
		</div>
	{/if}
</section>
	...
	<section class="space-y-2">
		...
		<Card padding="sm" class="space-y-2">
			{#each config.applications as rule, index (index)}
				<ListRow
					title={rule.match.desktopEntry ?? rule.match.appId ?? rule.match.atspiName ?? 'Matches nothing'}
		...
		<p class="px-1 text-2xs text-faint">
			More specific wins: desktop entry, then app id, then name. A deny outranks every allow, wherever it sits in this
			list. Stored in ~/.config/nib/desktop-agent.json.
		</p>
	</section>
{/if}
