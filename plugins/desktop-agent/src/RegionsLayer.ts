...
export class RegionsLayer {
  ...
  sync(): void {
    ...
    for (const entry of drawable) {
      this.graphics.rect(entry.x, entry.y, entry.width, entry.height).stroke({
        width: entry.sensitive ? 2 : 1.5,
        color: entry.sensitive ? SENSITIVE : ACCENT,
        alpha: 0.9,
      });
      if (entry.sensitive) {
        ...
        this.graphics
          ...
          .fill({ color: SENSITIVE, alpha: 0.18 });
      }
    }
  }
