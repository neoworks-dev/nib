...
export function threshold(edges: Uint8ClampedArray, cutoff: number): Uint8Array {
	...
	for (let index = 0; index < edges.length; index += 1) mask[index] = edges[index]! >= cutoff ? 1 : 0;
	return mask;
}
...
function sweep(mask: Uint8Array, width: number, height: number, radius: number, wanted: 0 | 1): Uint8Array {
...
function score(component: Component): number {
	...
	const aspect = Math.min(component.width, component.height) / Math.max(component.width, component.height);
...
export function detectRegions(bitmap: Bitmap, options: DetectorOptions = {}): DetectedRegion[] {
	...
	const mask = close(threshold(sobel(greyscale(bitmap), width, height), settings.threshold), width, height);
