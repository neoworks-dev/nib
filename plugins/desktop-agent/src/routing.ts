...
function levelOf(match: ApplicationMatch, identity: ApplicationIdentity): MatchLevel | null {
  ...
  if (match.desktopEntry && identity.desktopEntry && match.desktopEntry === identity.desktopEntry)
    return "desktopEntry";
  ...
  if (match.appId === "*" || match.atspiName === "*" || match.desktopEntry === "*")
    return "wildcard";
  return null;
}
...
export function resolveRouting(
  ...
  for (const rule of config.applications) {
    ...
    if (!best || MATCH_LEVELS.indexOf(level) < MATCH_LEVELS.indexOf(best.level))
      best = { rule, level };
  }
...
function parsePolicy(raw: unknown, fallback: CapturePolicy): CapturePolicy {
  return typeof raw === "string" && POLICIES.has(raw as CapturePolicy)
    ? (raw as CapturePolicy)
    : fallback;
}
...
function parseMatch(raw: unknown): ApplicationMatch | null {
  ...
  const match: ApplicationMatch = {
    ...(typeof candidate.appId === "string" &&
      candidate.appId.length > 0 && { appId: candidate.appId }),
    ...(typeof candidate.atspiName === "string" &&
      candidate.atspiName.length > 0 && { atspiName: candidate.atspiName }),
