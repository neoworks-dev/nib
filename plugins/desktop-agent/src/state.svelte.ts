...
import { parseDesktopRegions, REGIONS_KIND, regionsFor, type DesktopRegionsObject } from './objects';
import { composeDesktopPrompt } from './prompt';
import { clipToImage, imageToLayout, layoutToImage, mergeRegions } from './regions';
import { RegionsLayer, REGIONS_LAYER_ID } from './RegionsLayer';
import { emptyRoutingConfig, resolveRouting, type RoutingConfig } from './routing';
...
class DesktopState implements DesktopAgentService {
	...
	get captureBlocker(): string | null {
		if (!this.bridge) return 'Desktop control needs the desktop app; a browser has no access to the screen.';
	...
	overlayBlocker(objectId: string): string | null {
		...
		if (stored.regions.length === 0) return 'Run “Find elements” first: there is nothing to point at yet.';
		if (!stored.layout) {
	...
	async capture(request: CaptureRequest): Promise<string | null> {
		...
		try {
			...
			if (!result) {
				this.error = 'The capture did not happen: the desktop refused it, or no capture tool answered.';
				return null;
			}
	...
	async detect(objectId: string): Promise<DetectedRegion[]> {
		...
		try {
			...
			if (regions.length === 0) {
				this.error = 'Nothing was found in this capture: no accessibility tree, and no shapes with enough contrast.';
			}
			return regions;
		} finally {
			this.busy = false;
	...
	async showOverlay(objectId: string): Promise<void> {
		...
		const shown = await this.bridge.showOverlay({
			regions,
			...(this.capabilities?.pointer !== 'unavailable' && { pointer: { radius: 22, tone: 'muted' as const } }),
		});
		this.overlaid = shown ? objectId : null;
		if (!shown) this.error = 'The compositor refused a layer surface, so the overlay cannot be drawn.';
	}
	...
	async sendToHarness(objectId: string, text: string): Promise<string | null> {
		...
		try {
			...
			await sessions.create({
				...
				cwd: canvas.cwd,
				...(decision.options && Object.keys(decision.options).length > 0 && { options: decision.options }),
			});
	...
	mountLayer(canvas: CanvasRegistry): Disposer {
		...
		const unregister = canvas.registerLayer({ id: REGIONS_LAYER_ID, container: layer.container, order: 1 });
