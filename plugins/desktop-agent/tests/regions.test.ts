...
const placement: CapturePlacement = { x: 0, y: 0, width: 960, height: 540, imageWidth: 1920, imageHeight: 1080 };

function region(id: string, rect: DetectedRegion['rect'], extra: Partial<DetectedRegion> = {}): DetectedRegion {
...
describe('worldRectFor', () => {
	...
	test('returns null when the capture has no intrinsic size', () => {
		expect(worldRectFor({ ...placement, imageWidth: 0 }, { x: 0, y: 0, width: 1, height: 1 })).toBeNull();
	});
});
...
describe('intersectionArea', () => {
	...
	test('disjoint rects', () => {
		expect(intersectionArea({ x: 0, y: 0, width: 5, height: 5 }, { x: 50, y: 50, width: 5, height: 5 })).toBe(0);
	});
...
describe('mergeRegions', () => {
	const named = region('save', { x: 0, y: 0, width: 100, height: 40 }, { source: 'atspi', label: 'Save' });
	...
	test('accessible regions never suppress each other, because a real tree nests', () => {
		const panel = region('panel', { x: 0, y: 0, width: 400, height: 300 }, { source: 'atspi', label: 'Toolbar' });
...
describe('describeRegion', () => {
	test('names a labelled region and rounds its box', () => {
		expect(describeRegion(region('a', { x: 10.4, y: 20.6, width: 100.2, height: 30.9 }, { label: 'Save' }))).toBe(
			'Save — 10,21 100×31',
		);
	});

	test('falls back to the role, then to unlabelled', () => {
		expect(describeRegion(region('a', { x: 0, y: 0, width: 1, height: 1 }, { role: 'push button' }))).toBe(
			'push button — 0,0 1×1',
		);
		expect(describeRegion(region('a', { x: 0, y: 0, width: 1, height: 1 }))).toBe('unlabelled — 0,0 1×1');
	});
});
