...
describe('resolveRouting precedence', () => {
	...
	const full = identity({ appId: 'org.blender.Blender', atspiName: 'Blender', desktopEntry: 'blender.desktop' });
	...
	test.each([
		...
		['atspi substring', ['desktopEntry', 'appId', 'appIdPrefix', 'atspiName'], 'atspiSubstring', 'atspi-substring'],
		...
		const remaining = rules.filter((rule) => {
			...
			if (removed.includes('appId') && rule.match.appId && !rule.match.appId.endsWith('.')) return false;
	...
	test('a wildcard is the last rule tried, ahead of the default', () => {
		const decision = resolveRouting(identity({ appId: 'unknown.app' }), config([{ match: { appId: '*' }, policy: 'ask', harnessId: 'catch-all' }]));
	...
	test('a prefix rule needs the trailing dot; a bare prefix does not match', () => {
		const decision = resolveRouting(identity({ appId: 'org.mozilla.firefox' }), config([{ match: { appId: 'org.mozilla' }, policy: 'allow' }]));
		expect(decision.matchedBy).toBeNull();
	});

	test('atspi matching is case-insensitive both ways', () => {
		const decision = resolveRouting(identity({ atspiName: 'BLENDER' }), config([{ match: { atspiName: 'blender' }, policy: 'allow', harnessId: 'x' }]));
...
describe('resolveRouting fallbacks', () => {
	test('an unknown application gets the default and asks', () => {
		...
		expect(decision).toMatchObject({ policy: 'ask', harnessId: null, matchedBy: null, rule: null, options: {} });
	});

	test('an identity with nothing in it matches nothing', () => {
		const decision = resolveRouting(identity(), config([{ match: { appId: 'a' }, policy: 'allow' }]));
		expect(decision.matchedBy).toBeNull();
	});

	test('a null identity falls back without throwing', () => {
		expect(resolveRouting(null, config([{ match: { appId: '*' }, policy: 'allow' }])).matchedBy).toBeNull();
	});

	test('a rule with no harness of its own inherits the default harness', () => {
		const decision = resolveRouting(
			identity({ appId: 'a' }),
			config([{ match: { appId: 'a' }, policy: 'allow' }], { policy: 'ask', harnessId: 'claude-code' }),
		);
	...
	test('carries the adapter options and the prefix through unchanged', () => {
		...
		const decision = resolveRouting(
			...
			config([{ match: { appId: 'blender' }, policy: 'allow', promptPrefix: 'Help in Blender.', options, skill: 'modelling' }]),
		);
...
describe('parseRoutingConfig', () => {
	...
	test('an unknown policy reads as ask rather than as allow', () => {
		const parsed = parseRoutingConfig({ applications: [{ match: { appId: 'a' }, policy: 'always' }] });
	...
	test('an options field that is not an object is dropped, not coerced', () => {
		const parsed = parseRoutingConfig({ applications: [{ match: { appId: 'a' }, policy: 'allow', options: ['x'] }] });
...
describe('mergeRoutingConfig', () => {
	...
	test('replaces the rules wholesale rather than merging them', () => {
		...
		const merged = mergeRoutingConfig({ applications: [{ match: { appId: 'a' }, policy: 'deny' }] }, next);
