...
<button
	type="button"
	class="flex w-full items-center gap-1.5 py-0.5 pr-2 text-left text-xs hover:bg-hover {activePath === entry.path
		? 'bg-raised'
		: ''}"
...
>
	...
	{#if statusMark(entry.status)}
		<span class="ml-auto shrink-0 font-mono text-2xs {statusTone(entry.status)}">{statusMark(entry.status)}</span>
	{/if}
</button>
