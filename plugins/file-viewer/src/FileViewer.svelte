<script lang="ts">
	...
	import { selectionPrompt, sliceLines, type Selection } from './ask';
	...
	function endSelect() {
		...
		if (selection && active) selection = { ...selection, text: sliceLines(active.text, selection.from, selection.to) };
	}
			...
			>
				{#each lines as line, index (index)}
					...
					>
						...
						{:else}
							<code class="flex-1 px-1 py-px whitespace-pre">{@html markup[line.number - 1] || ' '}</code>
						{/if}
					</div>
				{/each}
			</div>
