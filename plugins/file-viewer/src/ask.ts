...
export function describeRange(selection: Selection): string {
	return selection.from === selection.to ? `line ${selection.from}` : `lines ${selection.from}–${selection.to}`;
}
 ...
 */
export function selectionPrompt(path: string, selection: Selection, request: string): string | null {
	const question = request.trim();
	if (question.length === 0) return null;

	const lines = selection.text.replace(/\s+$/, '').split('\n');
	const snippet = lines.length > maxSnippetLines ? [...lines.slice(0, maxSnippetLines), '…'] : lines;
