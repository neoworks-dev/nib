import { blockToolInput, findToolResultBlock, type MessageView, type SessionView } from '@nib-ui/protocol';
...
export function reviewLines(text: string, hunks: Hunk[]): ReviewLine[] {
	...
	for (let index = 0; index < source.length; index += 1) {
		...
		if (!hunk) {
			lines.push({ kind: 'context', number: index + 1, text: source[index]!, editId: null, startsHunk: false });
			continue;
		}

		const first = lines.length;
		for (const removed of hunk.removed) {
			lines.push({ kind: 'removed', number: null, text: removed, editId: hunk.editId, startsHunk: false });
		}
		for (let offset = 0; offset < hunk.added.length && index + offset < source.length; offset += 1) {
			lines.push({
				kind: 'added',
