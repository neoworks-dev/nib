...
describe('splitMarkup', () => {
	test('closes and reopens a span that covers several lines', () => {
		...
		expect(lines).toEqual(['<span class="hljs-comment">/* one</span>', '<span class="hljs-comment">two */</span>']);
	});

	test('leaves single-line markup untouched', () => {
		expect(splitMarkup('<span class="hljs-keyword">const</span> x')).toEqual(['<span class="hljs-keyword">const</span> x']);
	});
});
