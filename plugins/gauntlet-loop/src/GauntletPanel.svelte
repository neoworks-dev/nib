<script lang="ts">
  ...
  import { nextHandoff, type PaneRole, peerRole, relayModes } from "./relay";
  ...
  const builder = $derived(
    gauntletState.builderId ? gauntletState.sessions?.view(gauntletState.builderId) : null,
  );
  const critic = $derived(
    gauntletState.criticId ? gauntletState.sessions?.view(gauntletState.criticId) : null,
  );
...
<div class="flex h-full min-h-0 flex-col bg-canvas">
  <header class="flex items-center gap-2 border-b border-line px-4 py-2.5">
    ...
    {#if lastRelay}
      <span class="rounded-full bg-raised px-2 py-0.5 text-2xs text-muted"
        >last handoff: {lastRelay}</span
      >
    {/if}
  </header>
  ...
  <div class="flex min-h-0 flex-1 items-stretch gap-0">
    ...
    <div
      class="flex w-16 shrink-0 flex-col items-center justify-center gap-2 border-y border-line bg-elevated"
    >
