<script lang="ts">
	...
	const {
		...
		title,
		tone,
	}: { role: PaneRole; title: string; tone: 'blue' | 'violet' } = $props();
...
<section class="flex min-h-0 min-w-0 flex-1 flex-col border border-line bg-canvas">
	<header class="flex items-center gap-2 border-b border-line px-3 py-2">
		<span class="text-2xs tracking-caps uppercase {tone === 'blue' ? 'text-blue' : 'text-violet'}">{title}</span>
		<select
			value={sessionId ?? ''}
		...
		<StatusBadge tone={status === 'error' ? 'red' : status === 'working' ? 'blue' : 'neutral'}>{status}</StatusBadge>
	</header>
