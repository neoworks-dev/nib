...
export const relayModes: { id: RelayMode; label: string; hint: string }[] = [
	...
	{ id: 'critic-to-builder', label: 'Critic → builder', hint: 'Critiques are forwarded to the builder' },
...
export function messageText(message: MessageView): string {
	return message.blocks
		...
		.map((block) => (block.content?.kind === 'text' ? (block.content as { text: string }).text : block.text))
		.join('\n')
		.trim();
}
 ...
 */
export function nextHandoff(mode: RelayMode, candidate: HandoffCandidate): { messageId: string; text: string } | null {
