...
describe("nextHandoff", () => {
  ...
  test("forwards a finished turn once", () => {
    const handoff = nextHandoff("critic-to-builder", {
      from: "critic",
      view: critic,
    ...
    expect(
      nextHandoff("critic-to-builder", { from: "critic", view: critic, relayedMessageId: "m1" }),
    ).toBeNull();
  });
  ...
  test("waits for the pane to go idle", () => {
    ...
    expect(
      nextHandoff("both", { from: "critic", view: working, relayedMessageId: null }),
    ).toBeNull();
  });
  ...
  test("stays silent when communication is off or the turn is empty", () => {
    ...
    expect(
      nextHandoff("both", {
        from: "builder",
        view: view([message("m1", "assistant", "  ")]),
        relayedMessageId: null,
      }),
    ).toBeNull();
  });
