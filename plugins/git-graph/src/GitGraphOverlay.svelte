<script lang="ts">
	...
	import { assignLanes, laneCount, type GraphEdge } from './graph';
	...
	const rowHeight = 26;
	const dotRadius = 3.5;
	const laneColours = ['var(--ctx-blue)', 'var(--ctx-violet)', 'var(--ctx-green)', 'var(--ctx-amber)', 'var(--ctx-red)'];
	...
	function edgePath(edge: GraphEdge): string {
		...
		if (edge.kind === 'merge') return `M ${from},0 C ${from},${middle * 0.6} ${to},${middle * 0.4} ${to},${middle}`;
					...
					>
						<svg width={graphWidth} height={rowHeight} aria-hidden="true">
							{#each row.edges as edge, index (index)}
								<path d={edgePath(edge)} fill="none" stroke={laneColour(edge.colourIndex)} stroke-width="1.5" />
							{/each}
							<circle
