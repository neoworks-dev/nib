<script lang="ts">
	...
	import { changeLabel, commitChanges, fetchDiff, fetchStatus, setStaged, type GitStatus } from './client';
				...
				{#each [{ title: 'Staged', files: staged, staged: true }, { title: 'Changes', files: unstaged, staged: false }] as group (group.title)}
					{#if group.files.length > 0}
						...
						{#each group.files as file (file.path)}
							<div
								class="flex items-center gap-2 px-3 py-1.5 text-xs {gitPanelState.selectedPath === file.path
									? 'bg-raised'
									: 'hover:bg-hover'}"
							>
								<button
