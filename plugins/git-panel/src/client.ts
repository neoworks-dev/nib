...
export function fetchDiff(sessionId: string, path: string, staged: boolean): Promise<{ patch: string }> {
	const params = new URLSearchParams({ path, staged: String(staged) });
	return request(`/api/sessions/${sessionId}/git/diff?${params}`);
}

export function setStaged(sessionId: string, paths: string[], staged: boolean): Promise<{ ok: boolean }> {
...
export function commitChanges(sessionId: string, message: string, paths: string[]): Promise<{ output: string }> {
