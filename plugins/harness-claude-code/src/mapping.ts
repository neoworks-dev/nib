...
/** Default registry id, and the `ext` namespace this vocabulary is published under. */
export const claudeCodeHarnessId = "claude-code";
...
export class ClaudeMessageMapper {
  ...
  constructor(
    ...
    private readonly harnessId: string = claudeCodeHarnessId,
  ) {}
    ...
    this.emit({
      ...
      data: failed
        ? {
            status: "error",
            detail:
              "result" in message ? String(message.result ?? message.subtype) : message.subtype,
          }
        : { status: "idle" },
      raw: message,
    });
  ...
  private passthrough(message: SDKMessage): void {
    ...
    this.emit({
      type: "ext",
      data: { ns: claudeCodeHarnessId, type: `${message.type}${subtype}`, data: message },
      raw: message,
    });
  }
  ...
  private finishMessage(messageId: string, stopReason: string | null): void {
    ...
    this.emit({
      ...
      data: { messageId, stopReason: stopReason ?? undefined },
    });
  }
