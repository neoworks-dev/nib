...
export function mapCodexEvent(state: CodexStreamState, event: unknown): CodexMapResult {
  ...
  switch (event.type) {
    ...
    case "turn.completed": {
      ...
      if (state.messageId)
        events.push({ type: "message.completed", data: { messageId: state.messageId } });
    ...
    case "turn.failed": {
      ...
      if (state.messageId) {
        events.push({
          ...
          data: { messageId: state.messageId, stopReason: "error" },
        });
      }
      events.push({
        type: "session.status",
        ...
        events: [
          {
            ...
            data: {
              status: "error",
              detail: typeof event.message === "string" ? event.message : "codex error",
            },
            raw: event,
          },
        ],
    ...
    default:
      return {
        state,
        events: [
          { type: "ext", data: { ns: codexHarnessId, type: event.type, data: event }, raw: event },
        ],
      };
  }
}
...
function mapTextItem(
  ...
  if (phase === "item.completed") {
    events.push({
      ...
      data: { blockId, content: { kind, text } },
      raw: event,
    });
