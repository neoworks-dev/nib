<script lang="ts">
  ...
  const failed = $derived(
    block.content?.kind === "tool_result" && (block.content as { isError?: boolean }).isError,
  );
...
{#if failed}
  <div
    class="flex items-start gap-2 rounded-lg border border-red bg-input px-3 py-2 font-mono text-xs text-red"
  >
