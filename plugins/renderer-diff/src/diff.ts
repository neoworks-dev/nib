...
export function diffLines(before: string, after: string): DiffLine[] {
	...
	for (; leftIndex < left.length; leftIndex += 1) lines.push({ kind: 'removed', text: left[leftIndex]! });
	for (; rightIndex < right.length; rightIndex += 1) lines.push({ kind: 'added', text: right[rightIndex]! });
	return lines;
}
