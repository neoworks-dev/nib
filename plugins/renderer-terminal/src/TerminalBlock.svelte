<script lang="ts">
  ...
  const foldedIntoCall = $derived(
    block.kind === "tool_result" && findToolUseBlock(session, block.toolUseId) !== null,
  );
  const result = $derived(
    block.kind === "tool_use" ? findToolResultBlock(session, block.toolUseId) : block,
  );
  ...
  const text = $derived(
    stripAnsi(typeof output === "string" ? output : output ? JSON.stringify(output, null, 2) : ""),
  );
  ...
  const phase = $derived(
    !block.completed ? "writing" : !result ? "running" : failed ? "failed" : "done",
  );
