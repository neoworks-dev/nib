...
export const rendererTerminalPlugin: Plugin = {
	...
	apply(ctx) {
		...
		ctx.effect(() =>
			renderers.register({ kind: 'tool_use', toolName: 'Bash', priority: 10, component: TerminalBlock }),
		);
		ctx.effect(() =>
			renderers.register({ kind: 'tool_result', toolName: 'Bash', priority: 10, component: TerminalBlock }),
		);
	},
};
