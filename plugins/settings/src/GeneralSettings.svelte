<script lang="ts">
  ...
  import { type ThemePreference, themePreferences } from "./settings";
...
<section class="space-y-2">
  ...
  <Card padding="sm" class="space-y-2">
    ...
    <ListRow
      ...
      subtitle={modeSource
        ? `Modes offered by ${modeSource.displayName}.`
      ...
      {#snippet trailing()}
        <div class="w-40">
          <Select
            ...
            onChange={(value) =>
              settingsState.update({ defaultPermissionMode: toOptionalId(value) })}
          />
        </div>
      {/snippet}
    </ListRow>
  ...
  <div class="flex items-center gap-3 px-1">
    <Button size="sm" variant="ghost" onclick={() => settingsState.reset()}
