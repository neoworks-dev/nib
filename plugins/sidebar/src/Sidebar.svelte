...
{#if sidebarState.collapsed}
  <aside
    class="flex h-full w-10 shrink-0 flex-col items-center rounded-xl border border-line bg-elevated py-2"
  >
    <button
      type="button"
    ...
    <div class="min-h-0 flex-1 overflow-y-auto px-1.5 pb-2">
      {#each projects as project (project.path)}
        <section class="mb-1">
          <button
            type="button"
            class="flex w-full items-center gap-2 rounded-lg px-1.5 py-1.5 text-left {project.path ===
            openPath
