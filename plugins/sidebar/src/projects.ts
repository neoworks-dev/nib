...
import { workspaceName, type BoardSummary, type BoardWorkstream, type SessionSummary } from '@nib-ui/ui-contracts';
...
function toRow(workstream: BoardWorkstream, cwd: string, summaries: SessionSummary[]): WorkstreamRow {
...
export function projectRows(
	...
	for (const directory of recentDirectories) {
		...
		rows.set(path, { path, name: workspaceName(path), workstreams: [], activeCount: 0, updatedAt: 0 });
	}
