...
import type { BoardSummary, CanvasRegistry, SessionsService, TransportService } from '@nib-ui/ui-contracts';
import { projectRows, type ProjectRow, type WorkstreamRow } from './projects';
...
class SidebarState {
	...
	get projects(): ProjectRow[] {
		return projectRows(this.boards, this.sessions?.summaries ?? [], this.sessions?.recentDirectories ?? []);
	}
	...
	attach(services: { transport: TransportService; sessions: SessionsService; canvas: CanvasRegistry }): void {
