...
export const taskProgressPlugin: Plugin = {
	...
	apply(ctx) {
		...
		ctx.effect(() => panes.register({ id: paneId, kind: 'tasks', title: 'Progress', icon: TargetIcon, component: ProgressPanel }));
		ctx.effect(() => renderers.register({ kind: 'tool_use', toolName, priority: 10, component: TodoBlock }));
		ctx.effect(() =>
			renderers.register({ kind: 'tool_result', toolName, priority: 10, component: TodoResultBlock }),
		);
		ctx.effect(() =>
...
export { isTodoBlock, parseTodos, readTodos, todoProgress, type TodoItem, type TodoProgress, type TodoStatus } from './todos';
