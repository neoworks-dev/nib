...
describe('parseTodos', () => {
	...
	test('falls back to the content when no active form is given', () => {
		expect(parseTodos(todoBlock('b1', [{ content: 'Ship it', status: 'pending' }]))[0]!.activeForm).toBe('Ship it');
	});

	test('treats an unknown status as pending', () => {
		expect(parseTodos(todoBlock('b1', [{ content: 'Ship it', status: 'blocked' }]))[0]!.status).toBe('pending');
	});
...
describe('todoProgress', () => {
	...
	test('has no current step once every step is done', () => {
		const progress = todoProgress(parseTodos(todoBlock('b1', [{ content: 'Done', status: 'completed' }])));
