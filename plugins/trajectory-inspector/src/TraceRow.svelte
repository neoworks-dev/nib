<script lang="ts">
  ...
  const { node, selected, onselect }: { node: TraceNode; selected: boolean; onselect: () => void } =
    $props();
...
>
  <span
    class="w-20 shrink-0 rounded px-1.5 py-px text-center text-2xs tracking-caps {badgeTone[
      node.kind
    ]}"
  >
    {node.badge}
