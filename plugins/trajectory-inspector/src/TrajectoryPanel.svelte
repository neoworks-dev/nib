<script lang="ts">
	...
	function matches(node: TraceNode): boolean {
		...
		return `${node.badge} ${node.title} ${node.detail} ${node.result ?? ''}`.toLowerCase().includes(query);
	}
</script>
...
{#if inspectorState.open}
	...
	>
		<header class="flex items-center gap-2 border-b border-line px-4 py-3">
			...
			<span class="text-2xs text-faint tabular-nums">{visible.length}/{trace.length} steps · {events.length} events</span>
			<button
				type="button"
		...
		<div class="min-h-0 flex-1 overflow-y-auto font-mono text-2xs">
			{#each visible as node (node.id)}
				<TraceRow
					...
					onselect={() =>
						(inspectorState.selectedEventId = inspectorState.selectedEventId === node.id ? null : node.id)}
				/>
			{/each}
			{#if visible.length === 0}
				<p class="px-4 py-3 font-sans text-xs text-dim">
					{events.length === 0 ? 'No events recorded for this session yet.' : 'No steps match the filter.'}
				</p>
			{/if}
		</div>
