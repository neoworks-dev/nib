...
export function indexBlockKinds(events: AnyAgentEvent[]): Map<string, string> {
	...
	for (const event of events) {
		if (isKnownEvent(event) && event.type === 'block.started') kinds.set(event.data.blockId, event.data.kind);
	}
	return kinds;
}

export function categorizeEvent(event: AnyAgentEvent, blockKinds: Map<string, string>): EventCategory {
...
export function filterEvents(events: AnyAgentEvent[], filter: EventFilter): AnyAgentEvent[] {
	...
	return events.filter((event) => {
		if (filter.categories.length > 0 && !filter.categories.includes(categorizeEvent(event, blockKinds))) return false;
