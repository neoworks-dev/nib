...
export function mergeDeltas(events: AnyAgentEvent[]): TrajectoryRow[] {
  ...
  for (const event of events) {
    ...
    if (
      !isDelta(event) ||
      !previous ||
      !isDelta(previous.event) ||
      previous.event.data.blockId !== event.data.blockId
    ) {
...
type DeltaEvent = AnyAgentEvent & {
  type: "block.delta";
  data: { blockId: string; textDelta?: string; inputJsonDelta?: string };
};
