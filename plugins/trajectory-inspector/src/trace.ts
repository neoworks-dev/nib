import { isKnownEvent, type AnyAgentEvent, type BlockContent } from '@nib-ui/protocol';

export type TraceKind = 'session' | 'message' | 'text' | 'thinking' | 'tool' | 'permission' | 'state' | 'log' | 'other';
		...
		switch (event.type) {
			...
			case 'session.meta': {
				const detail = [
					...
					.filter(Boolean)
					.join(' · ');
				push(base(event, { depth: 0, kind: 'state', badge: 'META', title: 'session.meta', detail, turn }));
				break;
			}
			case 'session.status': {
				push(
			...
			case 'block.started': {
				...
				const call = event.data.kind === 'tool_result' ? byToolUseId.get(event.data.toolUseId ?? '') : undefined;
				if (call) {
...
function base(
	event: AnyAgentEvent,
	overrides: Partial<TraceNode> & { depth: number; kind: TraceKind; badge: string; title: string; turn: number },
): TraceNode {
	return {
		id: event.id,
		detail: '',
