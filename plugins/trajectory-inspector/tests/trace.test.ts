...
const fixturePath = new URL('../../../packages/protocol/tests/fixtures/claude-session.jsonl', import.meta.url).pathname;
...
describe('buildTrace', () => {
	test('nests blocks under their message and folds deltas into them', async () => {
		...
		expect(trace.some((node) => node.events.some((event) => event.type === 'block.delta'))).toBe(true);
	});
	...
	test('keeps an unknown event type as its own row', () => {
		const trace = buildTrace([
			{ id: 'x1', sessionId: 's1', seq: 1, ts: 1, type: 'harness.telepathy', data: { thought: 'hi' } },
		]);
